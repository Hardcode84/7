//===- WaveAMDRegAllocAssignments.cpp - Regalloc assignments -------------===//
//
...
static LogicalResult buildRegAllocAssignmentClearPlans(
    ArrayRef<func::FuncOp> funcs, DenseSet<Operation *> &functionsToClear,
...
static LogicalResult
clearRegAllocAssignmentsImpl(ModuleOp root, ArrayRef<func::FuncOp> funcs) {
  DenseSet<Operation *> functionsToClear;
  DenseMap<Operation *, FunctionType> plannedTypes;
  if (failed(buildRegAllocAssignmentClearPlans(funcs, functionsToClear,
                                               plannedTypes)) ||
...
LogicalResult wave::clearRegAllocAssignments(ModuleOp root) {
  SmallVector<func::FuncOp> funcs;
  root.walk([&](func::FuncOp func) { funcs.push_back(func); });
  return clearRegAllocAssignmentsImpl(root, funcs);
}

LogicalResult wave::clearRegAllocAssignments(func::FuncOp func) {
  ModuleOp root = func->getParentOfType<ModuleOp>();
  if (!root)
    return func.emitError(
        "cannot clear register assignments without a parent module");
  return clearRegAllocAssignmentsImpl(root, ArrayRef<func::FuncOp>(func));
}
