...
static LogicalResult duplicateMarkedTransferOperands(
  ...
  for (auto [index, transfer] : llvm::enumerate(transfers)) {
    ...
    FailureOr<Value> duplicate = wave::regalloc_detail::materializeRegAllocCopy(
        builder, loc, transfer.operand->get());
    if (failed(duplicate))
      return failure();
...
splitOverlappingRegionCarryStorage(func::FuncOp func,
  ...
  for (const auto &branch : flow.getBranches()) {
    for (const RegAllocRegionFlow::TransferGroup &group :
      ...
      Operation *sourceOperation = group.sourceOperation;
      ...
      if (!group.source)
        ...
        continue;
      builder.setInsertionPoint(sourceOperation);
      if (failed(duplicateMarkedTransferOperands(
              builder, sourceOperation->getLoc(), transfers, needsCopy)))
        return failure();
    }
  }
  return success();
    ...
    for (auto [blockIndex, block] : llvm::enumerate(region)) {
      ...
      for (auto [opIndex, op] : llvm::enumerate(block)) {
        ...
        for (auto [regionIndex, nested] : llvm::enumerate(op.getRegions())) {
          ...
          Region *nestedRepetitiveRegion = enclosingRepetitiveRegion;
          if (flow.isRepetitive(&nested)) {
            parentRepetitiveRegions[&nested] = enclosingRepetitiveRegion;
            nestedRepetitiveRegion = &nested;
          }
          std::optional<unsigned> nestedExclusiveContext = exclusiveContext;
          if (flow.isExclusiveChoice(&nested)) {
            unsigned entryPosition = nested.empty() || nested.front().empty()
                                         ? record.position
                                         : ops.size();
            nestedExclusiveContext = exclusiveContexts.size();
            exclusiveContexts.push_back(
                {&op, record.position, entryPosition, exclusiveContext});
          }
          collectRegion(nested, path, nestedRepetitiveRegion,
                        nestedExclusiveContext);
          path.pop_back();
        }
...
setRegAllocTransformState(func::FuncOp func, Builder &builder,
  ...
  if (!wave::isRegAllocPreparationValid(func)) {
    if (failed(wave::prepareWaveAMDRegAllocIR(func, flow)))
      return failure();
    if (failed(splitOverlappingRegionCarryStorage(func, flow)))
      return failure();
