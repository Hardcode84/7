...
static LogicalResult buildGraph(const GreedyRegion &region,
  ...
  for (SmallVector<unsigned, 4> &preds : graph.predecessors)
    llvm::sort(preds);
  for (SmallVector<unsigned, 4> &preds : graph.ssaPredecessors)
    llvm::sort(preds);
  return success();
}
...
buildComputeIslandInfo(const GreedyRegion &region, const GraphTables &graph,
  ...
  return ComputeIslandInfo(scheduleModel.createRegionSession(
      ...
      graph.fillerMemoryKinds, graph.memoryNodes, graph.ssaPredecessors,
      config));
}
...
getReadyScheduleDynamicIssueFacts(const GreedyRegion &region,
  ...
  facts.computePriorityStall = preview->computePriorityStall;
  return facts;
}
...
static void recordReadyDecision(const ReadyScheduleDecision &decision,
  ...
  case ReadyScheduleSelectionKind::SteadyStateProducer:
  case ReadyScheduleSelectionKind::SteadyStateFiller:
    ++stats.steadyStateFills;
    break;
  case ReadyScheduleSelectionKind::VmemPrefetch:
    ++stats.vmemPrefetchMoves;
    break;
  case ReadyScheduleSelectionKind::LongLatencyVmemPrefetch:
    ++stats.vmemPrefetchMoves;
    ++stats.longLatencyVmemPrefetchMoves;
    break;
  case ReadyScheduleSelectionKind::ResourcePriority:
    ++stats.resourcePriorityMoves;
    break;
  case ReadyScheduleSelectionKind::ResourceStallFiller:
    ++stats.resourceStallFills;
    break;
  case ReadyScheduleSelectionKind::LatencyPriority:
    ++stats.latencyPriorityMoves;
    break;
  case ReadyScheduleSelectionKind::Baseline:
  case ReadyScheduleSelectionKind::Pressure:
  case ReadyScheduleSelectionKind::Proposal:
  case ReadyScheduleSelectionKind::GenericStallFiller:
  case ReadyScheduleSelectionKind::MemoryTokenConsumer:
  case ReadyScheduleSelectionKind::BarrierPairFiller:
  case ReadyScheduleSelectionKind::DmaPostBarrierFiller:
    break;
  }
}
...
// The model picks a class; the coordinator only executes its greedy step and
// enforces barrier rendezvous legality.
...
FailureOr<MultiWaveGreedyResults> MultiWaveGreedyCoordinator::run() {
  ...
  while (scheduled != total) {
    ...
    FailureOr<unsigned> classId = classSchedule.selectClass(classFacts);
    if (failed(classId))
      return failure();
    size_t added = 0;
...
replayMultiWaveOrders(const MultiWaveRegions &regions,
  ...
  auto state = std::make_unique<waveamdmachine::MultiWaveExecutionState>(
      arch, placements,
      scheduleModel.buildMultiWaveInstructionConfig(config, regions[0].first));
...
static FailureOr<MultiWaveGreedyResults> buildBoundedMultiWaveGreedyResults(
  ...
  if (decision->refinementStats) {
    for (unsigned classId : llvm::seq<unsigned>(kMultiWaveScheduleClassCount)) {
      GreedyStats &stats = result[classId].stats;
      stats.steadyStateIterations =
          decision->refinementStats->steadyStateIterations;
      stats.steadyStateRefinements =
          decision->refinementStats->steadyStateRefinements;
      stats.recurrenceModelMoves =
          decision->refinementStats->recurrenceModelMoves[classId];
    }
  }
  return result;
}
...
struct WaveAMDMachineSchedulePass
  ...
  processSpecialization(waveamdmachine::UniformIfOp uniformIf,
    ...
    if (failed(collected))
      return failure();
    FailureOr<SmallVector<waveamdmachine::WavePlacement, 8>> placements =
        scheduleModel.selectMultiWavePlacements(uniformIf);
