...
public:
  ...
  FailureOr<bool> hasPriorityStall(Operation *op) const {
    const waveamdmachine::InstructionExecutionState *single =
        std::get_if<waveamdmachine::InstructionExecutionState>(&state);
    FailureOr<waveamdmachine::InstructionStall> stall =
        single ? single->query(op)
               : std::get<waveamdmachine::MultiWaveCohortExecutionState>(state)
                     .queryPriorityStall(op);
    if (failed(stall))
      return failure();
    return stall->cycles != 0;
  }
...
static void addEdge(GraphTables &graph, unsigned src, unsigned dst,
  ...
  if (kind == EdgeKind::Ssa)
    graph.ssaPredecessors[dst].push_back(src);
...
static void sortGraphAdjacency(GraphTables &graph) {
  for (SmallVector<unsigned, 4> &successors : graph.successors)
    llvm::sort(successors);
  for (SmallVector<unsigned, 4> &predecessors : graph.predecessors)
    llvm::sort(predecessors);
  for (SmallVector<unsigned, 4> &predecessors : graph.ssaPredecessors)
    llvm::sort(predecessors);
}
...
static LogicalResult buildGraph(const GreedyRegion &region,
  ...
  graph.ssaPredecessors.resize(region.ops.size());
...
template <typename Root> static ValueOriginMap buildValueOriginMap(Root &root) {
  ...
  root.template walk<WalkOrder::PreOrder>([&](Operation *op) {
...
getReadyScheduleIssueFacts(const IssuePreview &preview) {
  ReadyScheduleIssueFacts facts;
  facts.operandWaitCycles = preview.operandWaitCycles;
  facts.memoryWaitCycles = preview.memoryWaitCycles;
  facts.functionalUnitWaitCycles = preview.fuWaitCycles;
  facts.issueWaitCycles = preview.issueWaitCycles;
  facts.cuIssueWaitCycles = preview.cuIssueWaitCycles;
  facts.cmaIssueWaitCycles = preview.cmaIssueWaitCycles;
  facts.coexecWindowWaitCycles = preview.coexecWindowWaitCycles;
  facts.issueCycle = preview.issueCycle;
  facts.hazardWaitInstructions = preview.hazardWaitInsts;
  return facts;
}
...
getReadyScheduleCandidateIssueFacts(const GreedyRegion &region,
  ...
  if (failed(preview))
    return failure();
  ReadyScheduleCandidateIssueFacts facts;
  facts.nextIssueCycle = preview->nextIssueCycle;
  facts.issues = preview->issues;
  facts.realInstruction = preview->realInst;
  facts.stalls = stalls(*preview);
  return facts;
}
...
buildComputeIslandInfo(const GreedyRegion &region, const GraphTables &graph,
  ...
  RegionScheduleGraphFacts facts{region.ops,        graph.predecessors,
                                 graph.successors,  graph.ssaPredecessors,
                                 graph.memoryKinds, graph.fillerMemoryKinds,
                                 graph.memoryNodes, graph.node};
  return ComputeIslandInfo(scheduleModel.createRegionSession(
      facts, buildNoInsts(region, staticInfo), config));
}

static void addReadyScheduleWorkStats(const ComputeIslandInfo &computeIslands,
                                      GreedyStats &stats) {
  ReadyScheduleWorkStats work = computeIslands.session.getWorkStats();
  ...
  stats.pressurePriorityMoves = work.pressureSelections;
}
...
static FailureOr<ReadyScheduleDynamicIssueFacts>
getReadyScheduleDynamicIssueFacts(const GreedyRegion &region,
                                  const IssueState &state, unsigned index) {
  FailureOr<IssuePreview> preview = previewIssue(state, region.ops[index]);
  if (failed(preview))
    return failure();
  ReadyScheduleDynamicIssueFacts facts;
  facts.issue = getReadyScheduleIssueFacts(*preview);
  facts.resource = preview->resource;
  facts.nextIssueCycle = preview->nextIssueCycle;
  facts.issues = preview->issues;
  facts.realInstruction = preview->realInst;
  facts.priorityStall = preview->priorityStall;
  return facts;
}
...
static FailureOr<ReadyScheduleResourceFacts> getReadyScheduleResourceFacts(
    unsigned candidate, unsigned baseline, const GreedyRegion &region,
    const IssueState &state, std::optional<IssuePreview> &baselinePreview) {
  if (!baselinePreview) {
    FailureOr<IssuePreview> preview = previewIssue(state, region.ops[baseline]);
    if (failed(preview))
      return failure();
    baselinePreview = *preview;
  }
  waveamdmachine::InstructionScheduleResourcePreview candidateResource =
      state.resources.preview(
          state.getStaticInfo(region.ops[candidate]).resource);
  FailureOr<bool> candidatePriorityStall =
      state.model.hasPriorityStall(region.ops[candidate]);
  if (failed(candidatePriorityStall))
    return failure();
  return ReadyScheduleResourceFacts{
      baselinePreview->resource, candidateResource,
      stallsComputePriority(*baselinePreview), *candidatePriorityStall,
      /*prioritize=*/false};
}

static void recordReadyBookkeeping(const ReadyScheduleDecision &decision,
                                   GreedyStats &stats,
                                   std::optional<unsigned> &resumeTarget,
                                   unsigned baseline) {
  if (decision.filledStall)
    ++stats.filledGaps;
  if (decision.filledBarrierMemoryStall)
    ++stats.filledBarrierMemoryGaps;
  if (decision.resumeBaseline)
    resumeTarget = baseline;
}

static bool hasNoReadyMoveStat(ReadyScheduleSelectionKind kind) {
  static constexpr std::array<ReadyScheduleSelectionKind, 7> kinds = {
      ReadyScheduleSelectionKind::Baseline,
      ReadyScheduleSelectionKind::Pressure,
      ReadyScheduleSelectionKind::Proposal,
      ReadyScheduleSelectionKind::GenericStallFiller,
      ReadyScheduleSelectionKind::MemoryTokenConsumer,
      ReadyScheduleSelectionKind::BarrierPairFiller,
      ReadyScheduleSelectionKind::DmaPostBarrierFiller,
  };
  return llvm::is_contained(kinds, kind);
}

static void requireNoReadyMoveStat(ReadyScheduleSelectionKind kind) {
  if (!hasNoReadyMoveStat(kind))
    llvm_unreachable("unhandled ready selection stats");
}

static void recordReadyMoveStats(ReadyScheduleSelectionKind kind,
                                 GreedyStats &stats) {
  if (kind == ReadyScheduleSelectionKind::SteadyStateProducer ||
      kind == ReadyScheduleSelectionKind::SteadyStateFiller) {
    ++stats.steadyStateFills;
    return;
  }
  if (kind == ReadyScheduleSelectionKind::VmemPrefetch ||
      kind == ReadyScheduleSelectionKind::LongLatencyVmemPrefetch) {
    ++stats.vmemPrefetchMoves;
    if (kind == ReadyScheduleSelectionKind::LongLatencyVmemPrefetch)
      ++stats.longLatencyVmemPrefetchMoves;
    return;
  }
  if (kind == ReadyScheduleSelectionKind::ResourcePriority) {
    ++stats.resourcePriorityMoves;
    return;
  }
  if (kind == ReadyScheduleSelectionKind::ResourceStallFiller) {
    ++stats.resourceStallFills;
    return;
  }
  if (kind == ReadyScheduleSelectionKind::LatencyPriority) {
    ++stats.latencyPriorityMoves;
    return;
  }
  requireNoReadyMoveStat(kind);
}

static void recordReadyDecision(const ReadyScheduleDecision &decision,
                                GreedyStats &stats,
                                std::optional<unsigned> &resumeTarget,
                                unsigned baseline) {
  if (!decision.candidate)
    return;
  recordReadyBookkeeping(decision, stats, resumeTarget, baseline);
  recordReadyMoveStats(decision.kind, stats);
}

static FailureOr<std::optional<unsigned>> findReadyAlternative(
    ReadySchedulePhase phase, const BitVector &ready, unsigned next,
  ...
  auto issueProvider = [&](unsigned candidate, ReadyScheduleTimeline timeline) {
    const IssueState *selectedState = &state;
    if (timeline == ReadyScheduleTimeline::Steady) {
      assert(steadyState && "steady issue requested without steady state");
      selectedState = steadyState;
    }
    return getReadyScheduleDynamicIssueFacts(region, *selectedState, candidate);
  };
  std::optional<IssuePreview> resourceBaseline;
  auto resourceProvider =
      [&](unsigned candidate) -> FailureOr<ReadyScheduleResourceFacts> {
    return getReadyScheduleResourceFacts(candidate, next, region, state,
                                         resourceBaseline);
  };
  ...
  FailureOr<ReadyScheduleDecision> decision =
      computeIslands.session.selectReady(
          phase, scheduled, next, ready, steadyState != nullptr,
          state.model.getScheduleModel(), issueProvider, resourceProvider,
          projectionProvider);
  if (failed(decision))
    return failure();
  recordReadyDecision(*decision, stats, resumeTarget, next);
...
static FailureOr<std::optional<unsigned>> findModelRecurrenceAction(
    ...
    const IssueState *steadyState, const BitVector &ready,
  ...
  auto baselineProvider = [&]() -> FailureOr<RecurrenceScheduleBaselineFacts> {
    FailureOr<GreedyCompletion> completion = buildGreedyBaselineCompletion(
        ...
        origins, computeIslands, noInsts);
...
findGreedyAction(const GreedyRegion &region, const IssueState &state,
                 const IssueState *steadyState, const BitVector &ready,
                 unsigned next, const BitVector &scheduled,
                 GreedyResult &result, const ValueOriginMap &origins,
                 const ComputeIslandInfo &computeIslands,
                 std::optional<unsigned> &resumeTarget) {
  if (!ready.test(next))
  ...
  FailureOr<std::optional<unsigned>> alternative = findReadyAlternative(
      ReadySchedulePhase::Normal, ready, next, region, state, steadyState,
      scheduled, origins, computeIslands, result.stats, resumeTarget);
...
static FailureOr<std::optional<GreedyStepStatus>> resumeGreedyStallTarget(
  ...
  FailureOr<std::optional<unsigned>> modelDecision =
      findReadyAlternative(ReadySchedulePhase::ResumeBaseline, ready,
                           *stallTarget, region, state, steadyState, scheduled,
                           origins, computeIslands, result.stats, stallTarget);
  if (failed(modelDecision))
    return failure();
...
static FailureOr<GreedyStepStatus> scheduleGreedyReadyAction(
    ...
    const ComputeIslandInfo &computeIslands, const BitVector &noInsts,
    std::optional<unsigned> &stallTarget) {
  FailureOr<std::optional<unsigned>> action =
      findGreedyAction(region, state, steadyState, ready, next, scheduled,
                       result, origins, computeIslands, stallTarget);
  if (failed(action))
    return failure();
  if (*action)
...
static FailureOr<GreedyStepStatus> buildGreedyStepWithoutRecurrence(
    ...
    const ValueOriginMap &origins, const ComputeIslandInfo &computeIslands,
    const BitVector &noInsts, std::optional<unsigned> &stallTarget) {
  FailureOr<GreedyStepPreparation> preparation = prepareGreedyStep(
      region, graph, state, steadyState, ready, scheduled, pending, result,
      origins, computeIslands, noInsts, stallTarget);
  ...
  return scheduleGreedyReadyAction(
      preparation->next, region, graph, state, steadyState, ready, scheduled,
...
static FailureOr<GreedyStepStatus> buildGreedyStep(
  ...
  FailureOr<std::optional<unsigned>> recurrence = findModelRecurrenceAction(
      preparation->next, region, graph, arch, config, state, steadyState, ready,
      scheduled, pending, result, origins, computeIslands, noInsts);
...
replayLoopOrder(const GreedyRegion &region, ArrayRef<unsigned> order,
  ...
  std::unique_ptr<IssueState> state = std::make_unique<IssueState>(
      arch, scheduleModel.buildInstructionConfig(config), staticInfo,
      region.block);
  ...
  auto buildProvider = [&](const SingleWaveScheduleBuildRequest &request)
    ...
    if (request.replaySteadyState) {
      FailureOr<std::unique_ptr<IssueState>> state = replayLoopOrder(
          ...
          config, origins, *staticInfo, scheduleModel);
      if (failed(state))
        return failure();
...
using MultiWaveGraphs = std::array<GraphTables, kMultiWaveScheduleClassCount>;
using MultiWaveOrders = MultiWaveScheduleOrders;
using MultiWaveRegionLists =
    std::array<SmallVector<GreedyRegion, 16>, kMultiWaveScheduleClassCount>;
...
using MultiWaveGreedyResults =
    std::array<GreedyResult, kMultiWaveScheduleClassCount>;

// Model picks class; coordinator enforces barrier rendezvous.
...
public:
  MultiWaveGreedyCoordinator(
    ...
    for (unsigned wave : llvm::seq<unsigned>(localState->getWaveCount())) {
      unsigned classId = scheduleModel.getMultiWaveClass(*localState, wave);
    ...
    for (unsigned classId : llvm::seq<unsigned>(kMultiWaveScheduleClassCount)) {
      ...
      ComputeIslandInfo computeIslands =
          buildComputeIslandInfo(regions[classId], graphs[classId],
                                 staticInfo[classId], config, scheduleModel);
  ...
  std::unique_ptr<IssueState> takeIssueState() {
    SmallVector<unsigned> waves =
        llvm::to_vector(llvm::seq<unsigned>(localState->getWaveCount()));
    auto result =
        std::make_unique<IssueState>(*localState, waves, staticInfo[0]);
    result->staticInfo = nullptr;
    result->completionCycle = result->model.getCurrentCycle();
    for (const auto &group : classes)
      result->completionCycle =
          std::max(result->completionCycle, group->state.completionCycle);
    return result;
  }
...
private:
  ...
  std::array<SmallVector<unsigned, 4>, kMultiWaveScheduleClassCount> classWaves;
  std::array<StaticIssueInfoMap, kMultiWaveScheduleClassCount> staticInfo;
  std::array<std::unique_ptr<GreedyOrderState>, kMultiWaveScheduleClassCount>
      classes;
  std::array<Operation *, kMultiWaveScheduleClassCount> waitingBarriers{};
...
MultiWaveGreedyResults MultiWaveGreedyCoordinator::takeResults() {
  ...
  for (unsigned classId : llvm::seq<unsigned>(kMultiWaveScheduleClassCount)) {
    ...
    addReadyScheduleWorkStats(state.computeIslands, state.result.stats);
...
FailureOr<MultiWaveGreedyResults> MultiWaveGreedyCoordinator::run() {
  ...
  while (scheduled != total) {
    std::array<MultiWaveClassScheduleFacts, kMultiWaveScheduleClassCount>
        classFacts;
    for (unsigned classId : llvm::seq<unsigned>(kMultiWaveScheduleClassCount)) {
      classFacts[classId].complete =
          classes[classId]->result.order.size() == regions[classId].ops.size();
      classFacts[classId].waitingAtBarrier =
          waitingBarriers[classId] != nullptr;
    }
    std::optional<unsigned> classId = classSchedule.selectClass(classFacts);
    if (!classId)
      return failure();
    size_t added = 0;
    if (failed(advanceClass(*classId, added)))
      return failure();
    classSchedule.recordClassAdvance(*classId);
    scheduled += added;
  }
...
public:
  MultiWaveOrderReplay(const MultiWaveRegions &regions,
                       ...
                       unsigned iterations)
      : regions(regions), orders(orders), origins(origins),
        scheduleModel(scheduleModel), state(state), iterations(iterations) {
...
replayMultiWaveOrders(const MultiWaveRegions &regions,
                      const MultiWaveOrders &orders, unsigned iterations,
                      const waveamdmachine::ArchData &arch,
                      const waveamdmachine::EventSimConfig &config,
                      const ValueOriginMap &origins,
                      const WaveAMDMachineScheduleModel &scheduleModel,
  ...
  auto state = std::make_unique<waveamdmachine::MultiWaveExecutionState>(
      arch, placements, scheduleModel.buildMultiWaveInstructionConfig(config));
  initializeMultiWaveState(*state, regions, origins, scheduleModel);
  MultiWaveOrderReplay replay(regions, orders, origins, scheduleModel, *state,
                              iterations);
...
static FailureOr<MultiWaveGreedyResults> buildMultiWaveGreedyResults(
  ...
  MultiWaveGreedyCoordinator coordinator(regions, graphs, origins,
                                         ...
                                         placements, steadyState);
  FailureOr<MultiWaveGreedyResults> result = coordinator.run();
  if (succeeded(result))
    outgoing = coordinator.takeIssueState();
  return result;
}

static void applyMultiWaveRefinementStats(
    MultiWaveGreedyResults &results,
    const MultiWaveScheduleRefinementStats &refinement) {
  for (unsigned classId : llvm::seq<unsigned>(kMultiWaveScheduleClassCount)) {
    GreedyStats &stats = results[classId].stats;
    stats.steadyStateIterations = refinement.steadyStateIterations;
    stats.steadyStateRefinements = refinement.steadyStateRefinements;
    stats.recurrenceModelMoves = refinement.recurrenceModelMoves[classId];
  }
}

// Simulated totals never veto an order.
static FailureOr<MultiWaveGreedyResults> buildBoundedMultiWaveGreedyResults(
    const MultiWaveRegions &regions, const MultiWaveGraphs &graphs,
    const waveamdmachine::ArchData &arch,
    const waveamdmachine::EventSimConfig &config, const ValueOriginMap &origins,
    const WaveAMDMachineScheduleModel &scheduleModel,
    ArrayRef<waveamdmachine::WavePlacement> placements, IssueState &outgoing) {
  SmallVector<MultiWaveGreedyResults, 4> results;
  SmallVector<std::unique_ptr<IssueState>> states;
  auto buildProvider = [&](const MultiWaveScheduleBuildRequest &request)
      -> FailureOr<MultiWaveScheduleCandidateFacts> {
    std::unique_ptr<waveamdmachine::MultiWaveExecutionState> replayed;
    if (request.replaySteadyState) {
      FailureOr<std::unique_ptr<waveamdmachine::MultiWaveExecutionState>>
          state =
              replayMultiWaveOrders(regions, request.steadyStateOrders,
                                    request.steadyStateIterations, arch, config,
    ...
    if (failed(candidate))
      return failure();
    MultiWaveScheduleCandidateFacts facts;
    facts.orders = getMultiWaveOrders(*candidate);
    facts.resultToken = results.size();
    for (unsigned classId : llvm::seq<unsigned>(kMultiWaveScheduleClassCount))
      facts.recurrenceModelMoves[classId] =
          (*candidate)[classId].stats.recurrenceModelMoves;
  ...
  for (unsigned classId : llvm::seq<unsigned>(kMultiWaveScheduleClassCount))
    operations[classId] = regions[classId].ops;
  FailureOr<MultiWaveScheduleDecision> decision =
      scheduleModel.selectMultiWaveSchedule(operations, buildProvider);
  if (failed(decision))
    return failure();
  assert(decision->resultToken < results.size() &&
         "schedule model returned an invalid multi-wave result token");
  MultiWaveGreedyResults result = std::move(results[decision->resultToken]);
  outgoing = std::move(*states[decision->resultToken]);
  if (decision->refinementStats)
    applyMultiWaveRefinementStats(result, *decision->refinementStats);
  return result;
}
...
struct WaveAMDMachineSchedulePass
  ...
  WalkResult processFunction(func::FuncOp func,
    ...
    FailureOr<WaveAMDMachineScheduleModel> scheduleModel =
        WaveAMDMachineScheduleModel::create(func, *arch.arch,
                                            modelConfig->wavefrontSize);
  ...
  WalkResult scheduleFunctionRegions(
    ...
    bindValueOrigins(state.model, scope.origins, nullptr);
  ...
  static void initializeScopeRegions(ScheduleScope &scope) {
  ...
  scheduleCandidates(waveamdmachine::MaterializationCandidatesOp wrapper,
    ...
    SmallVector<std::unique_ptr<ScheduleScope>> candidates;
    for (Region &body : wrapper.getCandidates()) {
      if (failed(prepareCandidate(body,
        ...
        return failure();
      candidates.back()->state = std::make_unique<IssueState>(incoming);
    ...
    LogicalResult result = failableParallelForEach(
        ...
        [&](std::unique_ptr<ScheduleScope> &candidate) {
          return scheduleScope(*candidate, *candidate->state, arch, config);
        });
    ...
    IssueState state = std::move(*candidates[winner]->state);
    ...
    bindValueOriginsFromDef(state.model, parent.origins, wrapper, nullptr);
    return state;
  }
  ...
  static void resetIssueState(IssueState &state, const ScheduleScope &scope,
                              const waveamdmachine::ArchData &arch,
                              const waveamdmachine::EventSimConfig &config) {
    StaticIssueInfoMap empty;
    state =
        IssueState(arch, scope.model->buildInstructionConfig(config), empty);
    state.staticInfo = nullptr;
    bindValueOrigins(state.model, scope.origins, nullptr);
  }

  LogicalResult scheduleScope(ScheduleScope &scope, IssueState &state,
                              const waveamdmachine::ArchData &arch,
                              const waveamdmachine::EventSimConfig &config) {
    for (Block &block : *scope.body) {
      if (&block != &scope.body->front())
        resetIssueState(state, scope, arch, config);
      if (failed(scheduleBlock(block, scope, state, arch, config)))
        return failure();
    }
    return success();
  }

  LogicalResult scheduleBlock(Block &block, ScheduleScope &scope,
                              IssueState &state,
                              const waveamdmachine::ArchData &arch,
                              const waveamdmachine::EventSimConfig &config) {
    ...
    for (size_t index = 0; index < ops.size();) {
      ...
      if (GreedyRegion *region = scope.starts.lookup(op)) {
        if (index &&
            !isa<waveamdmachine::MaterializationCandidatesOp>(ops[index - 1]))
          resetIssueState(state, scope, arch, config);
      ...
      if (failed(scheduleBoundary(op, scope, state, arch, config)))
        return failure();
      ++index;
    }
    return success();
  ...
  LogicalResult scheduleBoundary(Operation *op, ScheduleScope &scope,
                                 IssueState &state,
                                 const waveamdmachine::ArchData &arch,
                                 const waveamdmachine::EventSimConfig &config) {
    if (auto wrapper =
            dyn_cast<waveamdmachine::MaterializationCandidatesOp>(op)) {
      FailureOr<IssueState> winner =
          scheduleCandidates(wrapper, scope, state, arch, config);
      if (failed(winner))
        return failure();
      state = std::move(*winner);
      return success();
    }
  ...
  FailureOr<MultiWaveGreedyResults> buildMultiWaveOrder(
      ...
      const WaveAMDMachineScheduleModel &scheduleModel,
      ArrayRef<waveamdmachine::WavePlacement> placements,
      MachineScheduleStageTiming &timing, IssueState &outgoing) {
  ...
  LogicalResult processSpecializedRegion(
      waveamdmachine::UniformIfOp uniformIf, const MultiWaveRegions &regions,
      const waveamdmachine::ArchData &arch,
      const waveamdmachine::EventSimConfig &config,
      const ValueOriginMap &origins,
    ...
    FailureOr<MultiWaveGreedyResults> greedy =
        buildMultiWaveOrder(uniformIf, regions, graphs, arch, config, origins,
                            scheduleModel, placements, timing, outgoing);
    if (failed(greedy))
      return failure();
  ...
  processSpecialization(waveamdmachine::UniformIfOp uniformIf,
                        ...
                        const WaveAMDMachineScheduleModel &scheduleModel,
                        MachineScheduleStageTiming &timing,
                        std::string *diagnostics, IssueState &outgoing,
                        int64_t &cycles) {
    ...
    SmallVector<waveamdmachine::WavePlacement> placements =
        scheduleModel.getMultiWavePlacements();
    ...
    for (unsigned regionIndex : llvm::seq<unsigned>((*collected)[0].size())) {
      ...
      if (failed(processSpecializedRegion(uniformIf, regions, arch, config,
                                          ...
                                          timing, outgoing)))
        return failure();
      cycles += outgoing.completionCycle;
    }
    return success();
  }
};
  ...
  WalkResult reportFunction(func::FuncOp func, ArrayRef<GreedyRegion> regions,
    ...
    if (printCandidates) {
      FailureOr<WaveAMDMachineScheduleModel> built =
          WaveAMDMachineScheduleModel::create(func, arch,
                                              modelConfig->wavefrontSize);
      if (failed(built))
        return WalkResult::interrupt();
