...
buildComputeIslandInfo(const GreedyRegion &region, const GraphTables &graph,
  ...
  RegionScheduleSession session = scheduleModel.createRegionSession(
      region.ops, graph.predecessors, graph.successors, std::move(noInsts));
  ComputeIslandInfo info(std::move(session));
...
static LogicalResult describeComputeResourceCandidate(
  ...
  proposal.resource = {nextPreview.resource, resource,
                       stallsComputePriority(nextPreview), *priorityStall,
...
static FailureOr<unsigned> findReadyComputeResourceAlternative(
  ...
  if (failed(collectComputeResourceCandidates(
    ...
    return failure();
  BitVector noDiscoveries(region.ops.size());
  ReadyScheduleDecision localDecision = computeIslands.session.selectNext(
      scheduled, next, noDiscoveries, local.proposals,
      state.model.getScheduleModel());
  if (localDecision.candidate) {
    recordComputeResourceSelection(localDecision, stats);
  ...
  if (failed(collectComputeRecurrenceResourceCandidates(
    ...
    return failure();
  ReadyScheduleDecision recurrenceDecision = computeIslands.session.selectNext(
      scheduled, next, noDiscoveries, recurrence.proposals,
      state.model.getScheduleModel());
  if (recurrenceDecision.candidate)
    recordComputeResourceSelection(recurrenceDecision, stats);
