...
static bool isIdentityPackedSubtract(VPkAddF32Op add) {
  return !add.getClamp() && add.getOpSel() == 0 && add.getOpSelHi() == 3 &&
         add.getNegLo() == 2 && add.getNegHi() == 2;
}

static VPkMulF32Op matchDirectPackedMulSub(VPkAddF32Op add) {
  if (!isIdentityPackedSubtract(add))
    return {};
...
static bool combineDirectPackedMulSub(func::FuncOp func) {
  ...
  for (VPkAddF32Op add : candidates) {
    VPkMulF32Op mul = matchDirectPackedMulSub(add);
    if (!mul)
      continue;
...
static bool isVccCompare(Operation *op) {
  return op && op->hasTrait<traits::VCmpVccOp>();
}
