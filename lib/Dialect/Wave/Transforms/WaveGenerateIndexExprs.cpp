...
static bool canBuildSymbolicBinaryOp(BinaryOp op, bool allowI64Integers,
  ...
  case BinaryKind::DivUI:
  case BinaryKind::RemUI:
    // Symbolic floor and modulo are useful for compile-time divisors because
    // index lowering can strength-reduce them directly. A runtime divisor is
    // already represented by this SSA operation and has a shared lowering in
    // WaveExpandIntegerDivRem. Keep that result as an index-expression leaf
    // instead of recreating the operation as a dynamic symbolic node.
...
private:
  ...
  std::optional<LoopCarriedUpdate> getLoopCarriedUpdate(BlockArgument arg) {
    ...
    if (!isSymbolicValueType(arg.getType(), allowI64Integers))
      return std::nullopt;
    auto loop = dyn_cast_or_null<scf::ForOp>(arg.getOwner()->getParentOp());
    if (!loop)
      return std::nullopt;
