...
getExecutionItem(const wave::memory_lowering::GatherTransactionRequest &request,
  ...
  for (const wave::memory_lowering::MemoryTransactionBinding &binding :
    ...
    if (!workitem || workitem.getAxis() != 0 ||
        !hasSymbol(request.points.front().byteOffset, binding.name))
      continue;
    bool common = llvm::all_of(request.points, [&](const auto &point) {
      const wave::memory_lowering::MemoryTransactionBinding *candidate =
          findBinding(point, binding.name);
      WorkitemIdOp candidateWorkitem =
          candidate ? candidate->value.getDefiningOp<WorkitemIdOp>() : nullptr;
      return candidateWorkitem && candidateWorkitem.getAxis() == 0 &&
             hasSymbol(point.byteOffset, binding.name);
    });
    if (!common)
      continue;
    FailureOr<sym::ExprHandle> item = analysis.composeSymbol(binding.name);
    if (succeeded(item))
      return ExecutionItem{*item, binding.name};
  }
  return failure();
}
