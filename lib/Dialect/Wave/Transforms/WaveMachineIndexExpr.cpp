//===- WaveMachineIndexExpr.cpp - bucketizer for `wave.index_expr` --------===//
//
...
// cluster, factored out of `WaveMachine.cpp`. The selector's
// per-op `select*` methods (and the rest of WaveMachine selection)
// stay in `WaveMachine.cpp`; this file owns the `wave.index_expr`
...
// SGPR / VGPR / imm ops via `WaveMachineSelector`'s public codegen
// helpers.
//
//===----------------------------------------------------------------------===//
...
FailureOr<Value> materializeIxsRat(WaveMachineSelector &S, ::ixs_node *node,
                                   Operation *user) {
  if (ixs_node_rat_den(node) != 1)
...
FailureOr<Value> materializeIxsSym(WaveMachineSelector &, ::ixs_node *node,
                                   Operation *user,
...
FailureOr<Value> materializeIxsAddTerm(WaveMachineSelector &S, ::ixs_node *node,
...
FailureOr<Value> materializeIxsAdd(WaveMachineSelector &S, ::ixs_node *node,
                                   Operation *user,
                                   const llvm::StringMap<Value> &subs) {
  Location loc = user->getLoc();
...
FailureOr<Value> materializeIxsMulFactor(WaveMachineSelector &S,
...
FailureOr<Value> materializeIxsMul(WaveMachineSelector &S, ::ixs_node *node,
                                   Operation *user,
                                   const llvm::StringMap<Value> &subs) {
  Location loc = user->getLoc();
...
// re-materialize.
FailureOr<Value> materializeScaledInteger(WaveMachineSelector &S,
...
// denominators only.
FailureOr<Value> materializeIxsFloor(WaveMachineSelector &S, ::ixs_node *node,
                                     Operation *user,
                                     const llvm::StringMap<Value> &subs) {
...
FailureOr<Value> materializeIxsCeil(WaveMachineSelector &S, ::ixs_node *node,
                                    Operation *user,
                                    const llvm::StringMap<Value> &subs) {
  ::ixs_node *child = ixs_node_unary_arg(node);
  int64_t den = collectDenominator(child);
...
FailureOr<Value> materializeIxsMod(WaveMachineSelector &S, ::ixs_node *node,
                                   Operation *user,
                                   const llvm::StringMap<Value> &subs) {
...
TermKind classifyAdd(WaveMachineSelector &S, ::ixs_node *node,
...
TermKind classifyMul(WaveMachineSelector &S, ::ixs_node *node,
                     const llvm::StringMap<TermKind> &symKinds) {
...
std::optional<int64_t> evalConstantAdd(WaveMachineSelector &S,
                                       ::ixs_node *node) {
...
std::optional<int64_t> evalConstantMul(WaveMachineSelector &S,
                                       ::ixs_node *node) {
...
std::optional<int64_t> evalConstantFloorOrCeil(WaveMachineSelector &S,
                                               ::ixs_node *node) {
  ::ixs_node *child = ixs_node_unary_arg(node);
  int64_t den = collectDenominator(child);
  if (den <= 0)
...
std::optional<int64_t> evalConstantMod(WaveMachineSelector &S,
                                       ::ixs_node *node) {
...
// slot.
FailureOr<Value> materializeSummand(WaveMachineSelector &S, ::ixs_node *term,
...
bool tryConstFoldSummand(WaveMachineSelector &S, ::ixs_node *term,
...
// bucket.
LogicalResult bucketizeSummand(WaveMachineSelector &S, ::ixs_node *term,
                               ::ixs_node *termCoeff, Operation *user,
                               const llvm::StringMap<Value> &subs,
                               const llvm::StringMap<TermKind> &symKinds,
                               OffsetTriple &triple) {
...
// ---- public surface (declared in WaveMachineSelector.h) ----------------
