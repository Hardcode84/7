...
static bool canPeelScheduleTokenProducer(Operation *producer,
                                         ScheduleTokenOp consumer) {
  // Keep memory effects and token inputs on their producer.
  return producer && producer->getNumRegions() == 0 &&
         isMemoryEffectFree(producer) && wouldOpBeTriviallyDead(producer) &&
         llvm::none_of(producer->getOperandTypes(),
...
struct PeelDeadScheduleTokenProducers : OpRewritePattern<ScheduleTokenOp> {
  ...
  LogicalResult matchAndRewrite(ScheduleTokenOp op,
    ...
    for (Value value : op.getDependencies()) {
      ...
      if (canPeelScheduleTokenProducer(producer, op)) {
