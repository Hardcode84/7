...
module attributes {transform.with_named_sequence} {
  ...
  transform.named_sequence @waveamd_backend_lower(
    ...
    %rmeta = transform.apply_registered_pass "canonicalize" to %rm
        : (!transform.any_op) -> !transform.any_op
    %raselect = transform.apply_registered_pass "wave-convert-arith-selects" to %rmeta
        : (!transform.any_op) -> !transform.any_op
    %rdivnorm = transform.apply_registered_pass "wave-normalize-integer-div-rem" to %raselect
        : (!transform.any_op) -> !transform.any_op
    ...
    %rcse0 = transform.apply_registered_pass "cse" to %rcanon0
        : (!transform.any_op) -> !transform.any_op
    %rwrites = transform.apply_registered_pass "waveamd-deduplicate-writes" to %rcse0
        : (!transform.any_op) -> !transform.any_op
    %rbalance = transform.apply_registered_pass "wave-balance-reassociable-math" to %rwrites
        : (!transform.any_op) -> !transform.any_op
    ...
    %racanon = transform.apply_registered_pass "canonicalize" to %raclean
        : (!transform.any_op) -> !transform.any_op
    %ratokenselect = transform.apply_registered_pass "wave-convert-arith-selects" to %racanon
        : (!transform.any_op) -> !transform.any_op
    %rselect = transform.apply_registered_pass "wave-lower-token-selects" to %ratokenselect
        : (!transform.any_op) -> !transform.any_op
    ...
    %rwhere = transform.apply_registered_pass "wave-coalesce-where" to %rdlp
        : (!transform.any_op) -> !transform.any_op
    %rvariants = transform.apply_registered_pass "wave-materialize-memory-variants" to %rwhere
        : (!transform.any_op) -> !transform.any_op
    %r0 = transform.apply_registered_pass "waveamd-to-machine" to %rvariants
        : (!transform.any_op) -> !transform.any_op
