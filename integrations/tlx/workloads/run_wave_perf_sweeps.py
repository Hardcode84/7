...
TRITON_SOURCE_ROOT = (
    Path(os.environ["WAVE_TLX_TRITON_SOURCE_DIR"]).resolve()
    if "WAVE_TLX_TRITON_SOURCE_DIR" in os.environ
    else Path(triton.__file__).resolve().parents[2]
)
TLX_TUTORIAL_DIR = TRITON_SOURCE_ROOT / "third_party" / "tlx" / "tutorials"
if not TLX_TUTORIAL_DIR.is_dir():
    raise RuntimeError(
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=(
            "parallel compilation workers within each sweep; 0 disables precompilation; "
    ...
    parser.add_argument(
        ...
        help=(
            "warmups before each MXFP batched timing repeat; "
    ...
    parser.add_argument(
        ...
        help=(
            "launches per MXFP batched event span; "
    ...
    parser.add_argument(
        ...
        help=(
            "MXFP batched timing samples; " f"default: {DEFAULT_MXFP_TIMING_REPEATS}"
        ),
    )
    parser.add_argument(
        "--cache-dir",
    ...
    parser.add_argument(
        "--wave-opt", type=Path, help="wave-opt binary to use for all Wave compilations"
    )
    parser.add_argument(
        "--fail-fast", action="store_true", help="stop after the first failed sweep"
    )
    parser.add_argument(
...
def build_run_specs(args, cache_root):
    ...
    if "mxfp" in args.sweeps:
        script = str(TLX_TUTORIAL_DIR / "gfx9_gemm/intra_wave/a4w4/bench.py")
        for backend in ("llvm", "wave"):
...
def main():
    ...
    if "mxfp" in args.sweeps:
        if args.mxfp_timing_mode == "batched":
            ...
            print(
                f"MXFP timing: triton median, {args.warmup}ms warmup/{args.rep}ms timed"
            )
