...
from triton.language.extra.tlx.tutorials.gfx9_gemm import f16_inputs
from triton.runtime.jit import MockTensor

INPUT_MODES = f16_inputs.INPUT_MODES
DEFAULT_INPUT_SEED = f16_inputs.DEFAULT_INPUT_SEED
make_inputs = f16_inputs.make_inputs
...
def triton_source_root():
    ...
    if not (root / "third_party" / "tlx" / "tutorials").is_dir():
        raise RuntimeError(
            "TLX Wave workloads require a Triton source checkout; set "
...
def make_driver(provider):
    ...
    if provider == "wave":
        ...
        return backends["tlx_wave"].driver()
    return None
...
def launch_tutorial_matmul(
    module, version_dir, a, b, out=None, extra_compile_options=None
):
    assert a.shape[1] == b.shape[0], "Incompatible dimensions"
    if version_dir in WAVE_STRUCTURED_VERSIONS:
        assert (
            b.stride(0) == 1
        ), f"{version_dir} expects a K-contiguous transposed-B view"
    M, K = a.shape
    K, N = b.shape
    if out is None:
...
def precompile_shapes(args, version_dir, providers, sizes):
    ...
    print(
        f"\nPrecompiling {len(jobs)} provider/shape job(s) with {workers} worker(s):",
        flush=True,
    )
    ...
    if workers == 1:
        for provider, shape in jobs:
            compiled_shape, compiled_provider, elapsed = compile_provider_shape(
                provider, version_dir, shape, args.b_layout, args.cache_dir, args.arch
            )
            M, N, K = compiled_shape
            print(
                f"  compiled {compiled_provider} {M}x{N}x{K} in {elapsed:.1f}s",
                flush=True,
            )
        return
...
def main():
    ...
    parser.add_argument(
        ...
        default="normal",
        help=(
            "input distribution; hpl and rand-int reproduce hipBLASLt/Wave "
    ...
    parser.add_argument(
        ...
        default="triton",
        help=(
            "triton records every launch and clears L2; batched records one event span "
    ...
    parser.add_argument(
        ...
        help=(
            "warmup launches before each batched timing repeat; "
    ...
    parser.add_argument(
        "--arch", default=None, help="optional Triton runtime arch override"
    )
    ...
    if args.timing_mode == "triton":
        ...
        timing_summary = (
            f"batched median, {args.timing_repeats}x"
        ...
        for provider in providers:
            try:
                result = benchmark_provider(
                    args, provider, version_dir, a, b, ref, M, N, K
                )
