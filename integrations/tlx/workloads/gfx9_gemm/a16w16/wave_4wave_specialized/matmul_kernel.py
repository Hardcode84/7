...
def _load_a_phase(region, phase: tl.constexpr, ready):
    return tl.tuple([_load_a_fragment(region, phase, tile, ready) for tile in range(8)])


@triton.jit
def _load_b_phase(region, phase: tl.constexpr, ready):
    return tl.tuple([_load_b_fragment(region, phase, tile, ready) for tile in range(8)])
...
def _prefetch_tile_subpanels(
    ...
    a0_view = tlx.local_slice(smem_a[buffer], [0, 0, 0, 0, 0, 0], [1, 2, 1, 8, 16, 32])
    a0 = tlx.buffer_load_to_local(a0_view, a_ptr, a_subpanel0_off + tile_k * stride_ak)
    a1_early_view = tlx.local_slice(
        smem_a[buffer], [0, 0, 1, 0, 0, 0], [1, 2, 1, 8, 4, 32]
    )
    a1_early = tlx.buffer_load_to_local(
        a1_early_view, a_ptr, a_subpanel1_request_off + tile_k * stride_ak
    )
    ...
    b0_view = tlx.local_slice(smem_b[buffer], [1, 0, 0, 0, 0, 0], [1, 2, 1, 8, 32, 16])
    ...
    b0 = tlx.buffer_load_to_local(b0_view, b_ptr, b_subpanel0_off + tile_k * stride_bk)
    b1_early = tlx.buffer_load_to_local(
        b1_early_view, b_ptr, b_subpanel1_request_off + tile_k * stride_bk
    )
    tlx.async_load_commit_group(tokens=[a1_late1, a1_late2, a1_late3, b0, b1_early])
...
def _mma_range(a, b, acc, begin: tl.constexpr, end: tl.constexpr):
    ...
    a_layout: tl.constexpr = tlx.dot_operand_layout_encoding.make(0, mma_layout, 8)
    b_layout: tl.constexpr = tlx.dot_operand_layout_encoding.make(1, mma_layout, 8)
        ...
        [
            (
                ...
                if begin <= ordinal and ordinal < end
                    ...
                    (ordinal // 8) * 8
                    + ((ordinal % 8) if (ordinal // 8) % 2 == 0 else 7 - ordinal % 8)
                ]
            )
    ...
    return tl.tuple(
        [
            tlx.release_layout(
                emitted[
                    (index // 8) * 8
                    + ((index % 8) if (index // 8) % 2 == 0 else 7 - index % 8)
                ]
            )
...
def _compute_tile_and_refill(
    ...
    refill_a0_token = tlx.buffer_load_to_local(refill_a0, refill_a_ptr, a_subpanel0_off)
    ...
    tlx.async_load_commit_group(tokens=[refill_a0_token, refill_a1_early_token])
    ...
    refill_a1_late1_token = tlx.buffer_load_to_local(
        refill_a1_late1, refill_a_ptr, a_subpanel1_request_off + 32 * stride_am
    )
    refill_a1_late2_token = tlx.buffer_load_to_local(
        refill_a1_late2, refill_a_ptr, a_subpanel1_request_off + 64 * stride_am
    )
    refill_a1_late3_token = tlx.buffer_load_to_local(
        refill_a1_late3, refill_a_ptr, a_subpanel1_request_off + 96 * stride_am
    )
    refill_b0_token = tlx.buffer_load_to_local(refill_b0, refill_b_ptr, b_subpanel0_off)
    ...
    refill_b1_late1_token = tlx.buffer_load_to_local(
        refill_b1_late1, refill_b_ptr, b_subpanel1_request_off + 32 * stride_bn
    )
    refill_b1_late2_token = tlx.buffer_load_to_local(
        refill_b1_late2, refill_b_ptr, b_subpanel1_request_off + 64 * stride_bn
    )
    refill_b1_late3_token = tlx.buffer_load_to_local(
        refill_b1_late3, refill_b_ptr, b_subpanel1_request_off + 96 * stride_bn
    )
    tlx.async_load_commit_group(
        tokens=[
    ...
    return (
        acc,
        (na0, na1, na2, na3, na4, na5, na6, na7),
...
@triton.jit
def _store_acc_strip(
    c_ptr, acc, pid_m, pid_n, stride_cm, stride_cn, local_m: tl.constexpr
):
    ...
    acc = tl.tuple([tlx.require_layout(value, fragment_layout) for value in acc])
    ...
    offs_n = (
        pid_n * 256 + (within_tile_n // 16) * 128 + (within_tile_n % 16) * 8 + tile_n
    )
...
def wave_4wave_specialized(
    ...
    smem_b = tlx.local_alloc(
        (2, 2, 2, 8, 32, 16), tl.float16, 2, reuse=smem_a, layout=b_load_layout
    )
    ...
    a_subpanel0_off = (pid_m * BLOCK_M + a_row * 8 + a_tile) * stride_am + (
        a_phase * 32 + a_k
    ) * stride_ak
    a_subpanel1_request_off = (
        pid_m * BLOCK_M + 128 + a_request_row * 8 + a_tile
    ) * stride_am + (a_phase * 32 + a_k) * stride_ak
    ...
    b_subpanel0_off = (b_phase * 32 + b_k) * stride_bk + (
        pid_n * BLOCK_N + b_col * 8 + b_tile
    ) * stride_bn
    b_subpanel1_request_off = (b_phase * 32 + b_k) * stride_bk + (
    ...
    acc = _compute_tile(a0, b0, acc, current_a, current_b, current_ready)

    last_ready = tlx.async_load_wait_group(0)
    last_a0 = _load_a_phase(next_a, 0, last_ready)
    last_b0 = _load_b_phase(next_b, 0, last_ready)
    acc = _compute_tile(last_a0, last_b0, acc, next_a, next_b, last_ready)
