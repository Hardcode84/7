...
def parse_inter_wave_shape(text):
    ...
    if shape[2] < MIN_K:
        raise argparse.ArgumentTypeError(
            f"8-wave inter-wave MXFP requires K >= {MIN_K}"
        )
    return shape
...
def launch_matmul(a, b, a_scales, b_scales, out=None):
    ...
    c = (
        out
        if out is not None
        else torch.empty((M, N), device=a.device, dtype=torch.bfloat16)
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="8-wave inter-wave TLX gfx950 MXFP4 GEMM benchmark"
    )
    parser.add_argument(
        "--shape", action="append", type=parse_inter_wave_shape, default=None
    )
    ...
    if args.timing_mode == "triton":
        ...
        timing_summary = (
            f"batched median, {args.timing_repeats}x{args.timed_launches} timed launches, "
    ...
    print(
        f"{'M':>6s} {'N':>6s} {'K':>6s}  {'status':>10s}  {'max_err':>10s}  {backend:>17s}"
    )
    for M, N, K in sizes:
        ...
        print(
            f"{M:6d} {N:6d} {K:6d}  {status:>10s}  {max_err:10.4f}  {perf:>17s}",
            flush=True,
        )
