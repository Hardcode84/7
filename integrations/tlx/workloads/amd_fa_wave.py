...
MAX_QK_ABS_FOR_FINITE_F32_DOT = math.sqrt(
    torch.finfo(torch.float32).max / (128 * (1.0 + _QK_ACCUMULATION_ERROR))
)
...
class SoftmaxState:
    ...
    def prepare(
        ...
        if ADAPTIVE_REFERENCE:
            score_registers0, score_registers1, tile_max = (
                _prepare_adaptive_score_registers(
                    scores,
                    qk_scale,
                )
            )
...
def _accumulate_prefix_body(
    ...
    if ADAPTIVE_REFERENCE:
        exponentiated_scores, reference, all_rows_within_headroom = (
            state.prepare_adaptive_pending(
                next_scores,
                qk_scale,
            )
        )
...
def _validate_attention_inputs(q, k, v, causal, compiler_options):
    ...
    if reserved_options:
        ...
        raise TypeError(
            f"amd_fa_wave compiler options must not override reserved launch keys: {names}"
        )
    if causal:
    ...
    for name, tensor in (("q", q), ("k", k), ("v", v)):
        if tensor.ndim != 4:
            raise ValueError(
                f"{name} must be rank 4 (B, H, N, D), got rank {tensor.ndim}"
            )
    ...
    for name, tensor in (("k", k), ("v", v)):
        if tensor.device != q.device:
            raise ValueError(
                f"{name} must be on the same device as q (q={q.device}, {name}={tensor.device})"
            )
        if tensor.shape != q.shape:
            raise ValueError(
                f"{name} must have shape {tuple(q.shape)}, got {tuple(tensor.shape)}"
            )
    for name, tensor in (("q", q), ("k", k), ("v", v)):
    ...
    if batch <= 0 or heads <= 0:
        raise ValueError(
            f"q batch and head dimensions must be positive, got B={batch}, H={heads}"
        )
    if sequence < 256 or sequence % 256 != 0:
        raise ValueError(
            f"q sequence length must be a multiple of 256 and at least 256, got N={sequence}"
        )
    if head_dim != 128:
    ...
    if batch_stride > MAX_SIGNED_I32 or last_element_offset > MAX_SIGNED_I32:
        raise ValueError(
            f"amd_fa_wave batch stride ({batch_stride}) and last element offset "
...
def _resolve_softmax_reference(head_dim, sm_scale, qk_max_abs):
    ...
    if qk_max_abs >= MAX_QK_ABS_FOR_FINITE_F32_DOT:
        raise ValueError(
            f"qk_max_abs ({qk_max_abs:g}) exceeds the conservative raw FP32 QK limit "
            f"({MAX_QK_ABS_FOR_FINITE_F32_DOT:g}) for head dimension 128"
        )
    log2_score_bound = (
        math.log2(math.e) * head_dim * abs(sm_scale) * qk_max_abs * qk_max_abs
    )
    log2_score_span = 2.0 * abs(log2_score_bound)
    if (
        not math.isfinite(log2_score_span)
        or log2_score_span >= FIXED_REFERENCE_MAX_LOG2_SPAN
    ):
        raise ValueError(
            f"fixed-reference log2 score span ({log2_score_span:g}) must be less than "
...
def _prepare_output(q, k, v, out):
    if out is None:
        ...
        if output.device != q.device:
            raise ValueError(
                f"out must be on the same device as q (q={q.device}, out={output.device})"
            )
        if output.shape != q.shape:
            raise ValueError(
                f"out must have shape {tuple(q.shape)}, got {tuple(output.shape)}"
            )
    ...
    if _storage_ranges_overlap(output, q) and not output.is_set_to(q):
        raise ValueError(
            "amd_fa_wave out may overlap q only when it is exactly the same tensor view"
        )
    return output
...
def attention(
    ...
    **compiler_options,
    ...
    """
    batch, heads, sequence, head_dim = _validate_attention_inputs(
        q, k, v, causal, compiler_options
    )
    sm_scale, adaptive_reference, log2_score_bound = _resolve_softmax_reference(
        head_dim, sm_scale, qk_max_abs
    )
