...
def forced_rebase_inputs(shape, device, *, seed):
    ...
    tile_indices = torch.arange(
        sequence // KV_TILE_SIZE, dtype=torch.int32, device=device
    )
    ...
    v = (
        tile_values.repeat_interleave(KV_TILE_SIZE)[None, None, :, None]
        .expand(shape)
        .contiguous()
    )
    ...
    if not bool(torch.all(advances > FORCED_REBASE_LOG2_HEADROOM)):
        raise ValueError(
            "forced-rebase score construction does not exceed the adaptive headroom"
        )
...
def check_correctness(q, k, v, *, qk_max_abs, case):
    ...
    print(
        f"  [{'PASS' if passed else 'FAIL'}] correctness {case} "
...
def main():
    ...
    mode = (
        ...
        else f"bounded (|Q|, |K| <= {args.qk_max_abs:g})"
    )
    ...
    if program_count != 4096 or program_count % XCD_COUNT != 0:
        raise RuntimeError(
            "advertised FA shape must exercise all 4096 programs across the eight-XCD swizzle"
        )
    print(
        f"Eight-wave {mode} FlashAttention ({backend}, programs={program_count}, XCDs={XCD_COUNT})"
    )
    correctness_ok = True
    performance_ok = True
    performance_cases = (
        ["bounded", "sparse", "forced"] if args.qk_max_abs is None else ["bounded"]
    )
    for input_case in performance_cases:
        if input_case == "forced":
            q, k, v, rebase_count, minimum_advance = forced_rebase_inputs(
                ADVERTISED_SHAPE, device, seed=17
            )
            case = f"forced-rebase count={rebase_count} min-log2-advance={minimum_advance:.6f}"
        elif input_case == "sparse":
            q, k, v, rebase_count, minimum_advance = sparse_rebase_inputs(
                ADVERTISED_SHAPE, device, seed=17
            )
            case = (
                f"sparse-rebase rows=1/{ROWS_PER_WAVE} count={rebase_count} "
        ...
        print(
            f"  [{'PASS' if case_ok else 'FAIL'}] performance {case} "
