...
def _kernel_runtime_args(a, b, bias, y, out):
    M, K = a.shape
    _, N = b.shape
    a_strides = a.stride()
    b_strides = b.stride()
...
def chiplet_transform_chunked(
    pid, num_workgroups, num_xcds: tl.constexpr, chunk_size: tl.constexpr
):
...
def _y_padded_layout(BM, BN, PAD):
    ...
    return tlx.padded_shared_layout_encoding.with_identity_for(
        [[BN, PAD]], [BM, BN], [1, 0]
    )
...
def tlx_addmm_glu_kernel_optimized(
    ...
    smemA = tlx.local_alloc(
        (BLOCK_SIZE_M, BLOCK_SIZE_K), tlx.dtype_of(a_ptr), NUM_BUFFERS
    )
    smemB = tlx.local_alloc(
        (BLOCK_SIZE_K, BLOCK_SIZE_N), tlx.dtype_of(b_ptr), NUM_BUFFERS
    )
    ...
    tok_a = tlx.async_load(
        a_ptr + a_offs, tlx.local_view(smemA, 0), mask=offs_k[None, :] < K - k_start
    )
    tok_b = tlx.async_load(
        b_ptr + b_offs, tlx.local_view(smemB, 0), mask=offs_k[:, None] < K - k_start
    )
    ...
    tok_a = tlx.async_load(
        a_ptr + a_offs, tlx.local_view(smemA, 1), mask=offs_k[None, :] < K - k_start
    )
    tok_b = tlx.async_load(
        b_ptr + b_offs, tlx.local_view(smemB, 1), mask=offs_k[:, None] < K - k_start
    )
    tlx.async_load_commit_group([tok_a, tok_b])
    ...
    tok_a = tlx.async_load(
        a_ptr + a_offs, tlx.local_view(smemA, 2), mask=offs_k[None, :] < K - k_start
    )
    tok_b = tlx.async_load(
        b_ptr + b_offs, tlx.local_view(smemB, 2), mask=offs_k[:, None] < K - k_start
    )
    tlx.async_load_commit_group([tok_a, tok_b])
...
def run_kernel_optimized(a, b, bias, y, out, cfg, *, warmup=False):
    M, K = a.shape
    _, N = b.shape
    ...
    return _launch_kernel(
        ...
        grid,
        _kernel_runtime_args(a, b, bias, y, out),
        {
...
def tlx_addmm_glu_kernel_optimized_async(
    ...
    for i in tl.range(0, NUM_BUFFERS, loop_unroll_factor=NUM_BUFFERS):
        ...
        tok_a = tlx.async_load(
            a_ptr + a_offs, tlx.local_view(smemA, i), mask=offs_k[None, :] < K - k_start
        )
        tok_b = tlx.async_load(
            b_ptr + b_offs, tlx.local_view(smemB, i), mask=offs_k[:, None] < K - k_start
        )
        tlx.async_load_commit_group([tok_a, tok_b])
    ...
    smemY = tlx.local_alloc(
        (BLOCK_SIZE_M, BLOCK_SIZE_N), tlx.dtype_of(y_ptr), 1, layout=y_layout
    )
    y_ptrs = y_ptr + offs_m[:, None] * sy0 + offs_n[None, :] * sy1
...
def run_kernel_optimized_async(a, b, bias, y, out, cfg, *, warmup=False):
    M, K = a.shape
    _, N = b.shape
    grid = (triton.cdiv(M, cfg["BLOCK_SIZE_M"]) * triton.cdiv(N, cfg["BLOCK_SIZE_N"]),)
    return _launch_kernel(
...
def tlx_addmm_glu_kernel_persistent(
    ...
    for tile_id in tl.range(start_pid, num_tiles, NUM_CUS):
        ...
        c_ptrs = c_ptr + offs_cm[:, None] * sc0 + offs_cn[None, :] * sc1
        tl.store(
            c_ptrs, out.to(c_ptr.dtype.element_ty), mask=c_mask, cache_modifier=".cs"
        )


def run_kernel_persistent(a, b, bias, y, out, cfg, num_cus, *, warmup=False):
    M, K = a.shape
    _, N = b.shape
    grid = (
        min(
...
def run_kernel_simple_async(a, b, bias, y, out, cfg, *, warmup=False):
    M, K = a.shape
    _, N = b.shape
    grid = (triton.cdiv(M, cfg["BLOCK_SIZE_M"]) * triton.cdiv(N, cfg["BLOCK_SIZE_N"]),)
    return _launch_kernel(
        tlx_addmm_glu_kernel_simple_async,
        grid,
        _kernel_runtime_args(a, b, bias, y, out),
        {
            "BLOCK_SIZE_M": cfg["BLOCK_SIZE_M"],
...
def run_kernel_baseline(bias, a, b, y, cfg, *, out=None, warmup=False):
    M, K = a.shape
    _, N = b.shape
    if out is None:
...
def compile_kernel_shape(kernel_name, shape, num_cus=None):
    M, N, K = shape
    a = MockTensor(torch.float16, [M, K])
    b = MockTensor(torch.float16, [K, N])
    bias = MockTensor(torch.float16, [N])
    y = MockTensor(torch.float16, [M, N])
    out = MockTensor(torch.float16, [M, N])

    if kernel_name == "tlx_baseline":
        run_kernel_baseline(
            bias,
            a,
            b,
            y,
            BASELINE_BEST_CONFIG[K],
            out=out,
            warmup=True,
        )
    elif kernel_name == "tlx_simple_async":
        run_kernel_simple_async(a, b, bias, y, out, SIMPLE_BEST_CONFIG[K], warmup=True)
    elif kernel_name == "tlx_optimized_async":
        run_kernel_optimized_async(
            a, b, bias, y, out, ASYNC_BEST_CONFIG[K], warmup=True
        )
    elif kernel_name == "tlx_optimized":
        run_kernel_optimized(a, b, bias, y, out, BEST_CONFIG[K], warmup=True)
    elif kernel_name == "tlx_persistent":
        if num_cus is None:
            raise ValueError("num_cus is required to precompile tlx_persistent")
        run_kernel_persistent(
            a,
            b,
            bias,
            y,
            out,
            PERSISTENT_BEST_CONFIG[K],
            num_cus,
            warmup=True,
        )
    else:
...
def compilation_jobs(args):
    if args.mode == "perf_test":
        return [
            (kernel_name, (M, N, K)) for kernel_name, K in sorted(PERF_BASELINE_TFLOPS)
        ]
    return [
        (kernel_name, (m, n, k))
        for m in args.M
...
def precompile_kernels(args):
    ...
    print(
        f"\nPrecompiling {len(jobs)} kernel/shape job(s) with {workers} worker(s):",
        flush=True,
    )
    ...
    if workers == 1:
        ...
        for kernel_name, shape in jobs:
            compiled_kernel, compiled_shape, elapsed = compile_kernel_shape_worker(
                kernel_name, shape, num_cus
            )
            m, n, k = compiled_shape
            print(
                f"  compiled {compiled_kernel} {m}x{n}x{k} in {elapsed:.1f}s",
                flush=True,
            )
        return
    ...
    with concurrent.futures.ProcessPoolExecutor(
        ...
        futures = [
            executor.submit(compile_kernel_shape_worker, kernel_name, shape, None)
            for kernel_name, shape in jobs
        ]
...
def measure_tflops(kernel_fn, M, N, K, warmup=25, rep=100):
...
def test_addmm_glu_performance(kernel_name, K):
    ...
    print(
        f"  {kernel_name:20s} K={K:5d} {got:7.1f} TFLOPS (baseline {baseline}, floor {floor:.1f})"
    )
    assert got >= floor, (
        f"perf regression: {kernel_name} K={K} -> "
...
def run_perf_test(args):
    ...
    for (kernel_name, K), baseline in sorted(PERF_BASELINE_TFLOPS.items()):
        ...
        try:
            got = measure_tflops(
                get_kernel(kernel_name), M, N, K, warmup=args.warmup, rep=args.rep
            )
        ...
        all_ok &= ok
        print(
            f"  [{'PASS' if ok else 'FAIL'}] {kernel_name:20s} K={K:5d} "
        ...
        for N in args.N:
            for K in args.K:
                ...
                ms = triton.testing.do_bench(
                    pt_lambda, warmup=args.warmup, rep=args.rep
                )
                results[key]["PyTorch_naive"] = {
                    "ms": ms,
                ...
                for kernel_name in args.kernel:
                    ...
                    try:
                        ...
                        print(
                            f"  {kernel_name:20s} M={M:5d} N={N:5d} K={K:5d} -> SKIPPED ({e})"
                        )
                        continue
                    ms = triton.testing.do_bench(
                        k_lambda, warmup=args.warmup, rep=args.rep
                    )
                    results[key][kernel_name] = {
                        "ms": ms,
                        "tflops": total_flops / ms * 1e-9,
                    }
                ...
                ms = triton.testing.do_bench(
                    rb_lambda, warmup=args.warmup, rep=args.rep
                )
                results[key]["ROCBLAS (matmul only)"] = {
                    "ms": ms,
                    "tflops": total_flops / ms * 1e-9,
                }
...
def parse_args():
    ...
    p.add_argument(
        "--rep", type=int, default=100, help="timed repetitions; default: 100"
    )
    p.add_argument(
        "--warmup", type=int, default=25, help="warmup iterations; default: 25"
    )
    p.add_argument(
        "--compile-workers",
        type=int,
...
if __name__ == "__main__":
    ...
    if args.rep <= 0 or args.warmup < 0:
        raise SystemExit("--rep must be positive and --warmup must be non-negative")
    precompile_kernels(args)
    if args.mode == "perf_test":
        run_perf_test(args)
    else:
        run_benchmark(args)
