...
def _add_wave_python_paths():
    paths = [str(path) for path in _existing_paths(_candidate_wave_python_paths())]
    selected = set(paths)
    sys.path[:] = paths + [path for path in sys.path if path not in selected]


@functools.cache
def backend_source_sha256():
