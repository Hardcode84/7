...
class _PacketShape:
    ...
    def __post_init__(self):
        ...
        if any(
            extent <= 0
            for extent in (self.block_count, self.item_count, self.slot_count)
        ):
...
def emit_wave_module(
    ...
    *,
    ...
    with dsl.ModuleBuilder() as module_builder:
        ...
        with module_builder.gpu_module("kernels") as gpu_module:  # noqa: SIM117
            with gpu_module.kernel(
                ...
                for target_value_id, arg in zip(
                    kernel.arg_target_ids, builder.args, strict=False
                ):
...
def _contains_waveamd_fragment(state, value):
    ...
    if isinstance(value, _VectorPacketPayload):
        return any(
            _contains_waveamd_fragment(state, packet) for packet in value.packets
        )
...
def _emit_binary(state, op):
    ...
    if result_type.representation in {"mask", "mask_tuple"}:
        ...
        lhs_components, rhs_components = _broadcast_components(
            state, (lhs, rhs), count, op
        )
        false_i1 = None
        false_mask = None
        reused = []
        components = []
        for lhs_component, rhs_component in zip(
            lhs_components, rhs_components, strict=False
        ):
            ...
            components.append(
                _reuse_component_result(
                    ...
                    lambda lhs_component=lhs_component, rhs_component=rhs_component, false_mask=false_mask: state.builder.select(
                        lhs_component,
                        rhs_component,
                        false_mask,
                    ),
...
def _emit_mma_packet_float_binary_component(
    ...
    if (
        ...
        or int(lhs_lane_width) != int(rhs_lane_width)
    ...
    if (
        operation in {"addf", "subf", "mulf"}
        and str(lhs_element_type) == "f32"
        and int(lhs_width) >= 2
...
def _emit_type_convert(state, op):
    ...
    if mode == "index_cast":
        ...
        result_type = state.target_program.values[result_id].type
        extension = (
            state.dsl.CastExtension.Sign
            if source_type.element_type == "i32" and result_type.element_type == "index"
            else None
        )
...
def _emit_assume(state, op):
    if (
        not op.fact_ids
        or len(op.operands) != len(op.results)
        or len(op.fact_target_ids) != len(op.fact_ids)
    ):
        fail(
    ...
    for operand_id, result_id in zip(op.operands, op.results, strict=False):
        components = _as_components(_require_value(state, operand_id, op))
        result_fact_ids = tuple(
            fact_id
            for fact_id, target_id in zip(op.fact_ids, op.fact_target_ids, strict=False)
        ...
        for fact_id in result_fact_ids:
            ...
            if fact.kind == "divisible":
                ...
                components = tuple(
                    state.builder.assume_divisible(component, int(fact.divisor))
                    for component in components
                )
                continue
            ...
            if assumptions:
                components = tuple(
                    state.builder.assume(component, assumptions, name="x")
                    for component in components
                )
        state.values[result_id] = _pack_components(components)
...
def _emit_make_range(state, op):
    ...
    block_id = (
        ...
        if int(state.target_program.kernel.num_ctas or 1) == 1
        else state.builder.cluster_workgroup_id()
    )
    bindings = {
        item: workitem,
        block: block_id,
...
def _emit_splat(state, op):
    ...
    if target_type.representation in {"mask", "mask_tuple"}:
        ...
        state.values[result_id] = _pack_components(
            tuple(mask for _ in range(_component_count(state, result_id)))
        )
        return
    lane_width = int(target_type.lane_width or 64)
...
def _emit_join(state, op):
    ...
    source_layouts = tuple(
        _packet_shape(state, source_id, op) for source_id in source_ids
    )
    source_packets = tuple(
        _pack_layout_value(state, op, source_id, value, layout)
        for source_id, value, layout in zip(
            source_ids, values, source_layouts, strict=False
        )
    )
...
def _emit_addptr(state, op):
    ...
    count = _component_count(state, result_id)
    base_components, offset_components = _broadcast_components(
        state, (base, offset), count, op
    )
    result_type = _wave_type(state.dsl, state.target_program.values[result_id].type)
    state.values[result_id] = _pack_components(
        tuple(
            ...
            for base_component, offset_component in zip(
                base_components, offset_components, strict=False
            )
        )
    )
...
def _target_value_is_live(target_program, target_value_id):
    ...
    return any(
        target_value_id in target_op.operands for target_op in target_program.ops
    ) or any(
        target_value_id in region.yield_value_ids for region in target_program.regions
    )
...
def _emit_barrier(state, op):
    ...
    completion_tokens = tuple(
        ...
        for target_value_id in completion_target_ids
    )
    issue_tokens = tuple(
        _require_value(state, target_value_id, op)
        for target_value_id in issue_target_ids
    )
...
def _workitem_id(state, axis, element_type, lane_width):
    ...
    if (
        axis == 0
        and lane_width == _kernel_threads_per_warp(state.target_program.kernel)
...
def _emit_warp_id(state, op):
    ...
    target_type = state.target_program.values[result_id].type
    lane_width = int(
        target_type.lane_width or state.target_program.kernel.threads_per_warp or 64
    )
...
def _select_vector_payload_components(
    ...
    payloads = tuple(
        _simd_1d_vector_payload(state, component) for component in true_components
    )
    ...
    for true_component, false_component, payload in zip(
        ...
        if false_payload is None or tuple(map(str, false_payload)) != tuple(
            map(str, payload)
        ):
            return None
...
def _emit_if_structural_only(state, op):
    ...
    observes_output = any(
        region_id in state.output_effect_regions for region_id in op.region_ids
    )
    if observes_output:
    ...
    cursor = 0
    for result_id, shape in zip(op.results, result_shapes, strict=False):
...
def _emit_for_loop_literal(state, op):
    ...
    loop_init_values = (
        (*flat_init_values, state.builder.token())
        if observes_output
        else flat_init_values
    )
...
def _structured_result_types_and_shapes(state, target_value_ids, op):
    ...
    shapes = []
    attrs = target_ir.attrs_dict(op)
    packet_registers = tuple(
        int(width) for width in attrs.get("result_packet_registers", ())
    )
    if packet_registers and len(packet_registers) < len(target_value_ids):
        ...
        if all(
            state.target_program.values[target_value_id].type.representation == "token"
...
def _flatten_structured_values(
    ...
    *,
    ...
    flat_values = []
    shapes = []
    for value, target_value_id in zip(values, target_value_ids, strict=False):
        target_type = state.target_program.values[target_value_id].type
        component_count = int(target_type.component_count)
        if isinstance(value, _VectorPacketPayload):
...
def _is_four_f32_simd_tuple(target_type):
    return (
        ...
        and target_type.element_type == "f32"
        and int(target_type.component_count) == 4
        and int(target_type.lane_width or 0) == 64
    )
...
def _preserve_loop_vector_payload_components(
    ...
    for component in components:
        ...
        elif payload_key != expected_payload or str(component.type) != str(
            expected_type
        ):
            fail(
                "TLXW_EMIT_FOR_MMA_PACKET",
...
def _pack_structured_value_components(state, components, shape, context, op):
    ...
    if shape.preserved_vector_payload_key is not None:
        ...
        for component in components:
            ...
            width, element_type, lane_width = payload
            payload_key = (int(width), str(element_type), int(lane_width))
            if payload_key != shape.preserved_vector_payload_key or str(
                component.type
            ) != str(shape.preserved_vector_payload_type):
                fail(
                    "TLXW_EMIT_STRUCTURED_COMPONENT_SHAPE",
                    STAGE,
...
def _symbolic_buffer_relation(state, op, attrs, ordinary_operand_count):
    ...
    target_ids = op.operands[
        int(ordinary_operand_count) : int(ordinary_operand_count) + count
    ]
    ...
    bindings = {
        ...
        for name, target_id in zip(names, target_ids, strict=False)
    }
...
def _emit_buffer_load_to_local(state, op):
    ...
    expected_operand_count = (
        ordinary_operand_count + binding_count + issue_dependency_count
    )
    if len(op.operands) != expected_operand_count:
        fail(
            "TLXW_EMIT_OPERAND_COUNT",
            STAGE,
            f"expected {expected_operand_count} operands, got " f"{len(op.operands)}",
            target_op_id=op.target_op_id,
        )
    ...
    if mask_components is not None and attrs.get("mask_mode") != "exec_where":
        fail(
            ...
            STAGE,
            f"unsupported buffer_load_to_local mask mode " f"{attrs.get('mask_mode')}",
            target_op_id=op.target_op_id,
        )
...
def _emit_async_commit_group(state, op):
    tokens = tuple(
        _require_value(state, target_value_id, op) for target_value_id in op.operands
    )
    token = state.builder.join(*tokens) if tokens else state.builder.token()
    if op.results:
        result_id = _single_result(op)
        state.values[result_id] = token
...
def _emit_lds_consumer_order(state, op):
    ...
    for target_value_id in op.operands:
        ...
        if target_value.type.representation == "token":
            ...
            continue
        dependencies.extend(
            state.builder.schedule_token(component)
            for component in _value_components(state, value, op)
        )
...
def _emit_async_wait(state, op):
    ...
    if (
        ...
        or completed_count + retained_issue_count != len(op.operands)
    ):
        fail(
...
def _deserialize_relation_expr(state, relation, op):
    try:
        if not relation or any(
            type(byte) is not int or not 0 <= byte <= 255 for byte in relation
        ):
...
def _mma_component_index(state, relation, axis0, axis1, component_count, op):
    ...
    _, normalized = state.dsl.ixs_check(
        (relation,),
        tuple(
            state.dsl.ixs_eq(axis, state.dsl.ixs_int(value))
            for axis, value in zip(axes, (axis0, axis1), strict=False)
        ),
    )
    try:
...
def _emit_mma(state, op):
    ...
    if (
        len(lhs_components) != m_tiles * k_tiles
        or len(rhs_components) != n_tiles * k_tiles
    ):
        fail(
            "TLXW_EMIT_COMPONENT_COUNT",
            STAGE,
    ...
    for m_tile in range(m_tiles):
        for n_tile in range(n_tiles):
            ...
            for k_tile in range(k_tiles):
                lhs_value = lhs_components[
                    _mma_component_index(
                        state,
...
def _select_packed_scale(scale_values, attrs, prefix, non_k_tile, k_tile, k_tiles, op):
    ...
    scale_idx = (int(non_k_tile) * int(k_tiles) + int(k_tile)) % (
        non_k_packed_vals * k_packed_vals
    )
...
def _emit_mma_packet_truncf(state, op):
    ...
    for regs in fragments:
        ...
        if (
            payload is None
            or int(payload[0]) != registers
            or str(payload[1]) != "f32"
...
def _packet_relation_exprs(
    ...
    *,
    ...
    try:
        ...
        source_item = state.dsl.mod(
            state.dsl.floor(packed / source_slots), source_items
        )
...
def _layout_packet_element_type(state, target_type, slot_count, op):
    ...
    if pointer_payload:
        ...
        address_space = (
            ...
            if target_type.representation in {"buffer_pointer", "buffer_pointer_tuple"}
...
def _packet_type(state, target_type, lane_width, slot_count, op):
    element_type, _mask_value = _layout_packet_element_type(
        state, target_type, slot_count, op
    )
    payload_type = (
        element_type
        if int(slot_count) == 1
        else state.dsl.vector_type(int(slot_count), element_type)
    )
...
def _pack_layout_value(state, op, target_value_id, value, layout):
    ...
    element_type, mask_value = _layout_packet_element_type(
        state, target_type, slot_count, op
    )
    if mask_value:
        components = tuple(
            _mask_to_redistribution_value(state, component, int(layout.lane_width))
            for component in components
        )
...
def _unpack_layout_value(state, op, target_value_id, packet, layout):
    ...
    if mask_value:
        components = tuple(
            _redistribution_value_to_mask(state, component, int(layout.lane_width))
            for component in components
        )
...
def _flatten_buffer_store_values(
    ...
    for component in logical_components:
        ...
        if str(payload_element_type) != str(element_type) or int(
            payload_lane_width
        ) != int(lane_width):
            fail(
        ...
        scalar_components.extend(
            state.dsl.wave.ExtractOp(scalar_type, component, element).result
            for element in range(int(width))
        )
    ...
    if target_type.representation in _MMA_PACKET_REPRESENTATIONS:
        ...
        return _pack_components(
            tuple(
                ...
                for index in range(0, int(access_component_count), payload_width)
            )
        )
    if payload_width != 1:
        fail(
...
def _emit_masked_effect_region(state, condition, emit_body):
    if not isinstance(condition, tuple | list) and _is_scalar_i1_value(
        state, condition
    ):
        with state.builder.if_(condition):
            emit_body()
...
def _same_component_operands(lhs, rhs):
    return len(lhs) == len(rhs) and all(
        ...
        for lhs_operand, rhs_operand in zip(lhs, rhs, strict=False)
    )
...
def _set_module_attrs(module_builder, dsl, ir, kernel):
    ...
    attrs["tlx_wave.num_ctas"] = ir.IntegerAttr.get(
        dsl.i32(), int(kernel.num_ctas or 1)
    )
    attrs["tlx_wave.num_warps"] = ir.IntegerAttr.get(
        dsl.i32(), int(kernel.num_warps or 1)
    )
