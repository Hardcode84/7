...
def build_layout_map(
    ...
    *,
    ...
    if kind in {"amd_mfma", "dot_operand"}:
        mma_model = _mma_component_model(
            kind,
        ...
        coordinate_domain = classify_coordinate_domain(
            source_type.shape, lane_width, linear=linear_layout
        )
        _require_supported_coordinate_domain(
            source_type.shape, coordinate_domain, value_id
        )
        ...
        component_count = _layout_component_count(
            source_type, lane_width, linear_layout
        )
    active_relation = (
        _distributed_active_relation(
            linear_layout,
...
def direct_to_lds_packet_layout(
    ...
    *,
    ...
    """
    if (
        distributed_layout is None
        or shared_layout is None
        or shared_layout.kind != "padded_shared"
    ):
        return None
    ...
    warp_count = int(warp_count)
    if (
        not _is_power_of_two(element_contiguity)
    ...
    def restrict_to_shape(bases):
        return tuple(
            tuple(
                int(component) if int(component) < shape[dim] else 0
...
def _layout_kind_and_properties(attr, value_id, context_owner, *, encoding=None):
    ...
    if _attr_bool(attr, "is_blocked_encoding"):
        return "blocked", {
            "size_per_thread": _int_tuple(
                _attr_value(attr, "get_blocked_size_per_thread")
            ),
            "threads_per_warp": _int_tuple(
                _attr_value(attr, "get_blocked_threads_per_warp")
            ),
    ...
    if _attr_bool(attr, "is_linear_encoding"):
        ...
        return kind, {
            "register_bases": _basis_tuple(
                _attr_value(attr, "get_linear_register_bases")
            ),
    ...
    if _attr_bool(attr, "is_amd_mfma_encoding"):
        return "amd_mfma", {
            ...
            "warps_per_cta": _int_tuple(
                _attr_value(attr, "get_amd_mfma_warps_per_cta")
            ),
            ...
            "tiles_per_warp": _int_tuple(
                _attr_value(attr, "get_amd_mfma_tiles_per_warp")
            ),
            "element_bit_width": int(
                _attr_value(attr, "get_amd_mfma_element_bit_width")
            ),
        }
...
def _mma_fragment_abi(
    ...
    *,
    ...
    if (
        spec is None
        or len(instr_shape) != 3
        or (op_idx is not None and op_idx not in (0, 1))
    ):
    ...
    if kind == "amd_mfma":
        ...
        if element_bits is None or element_type not in spec.operand_element_types:
            reject(
                "MFMA operand fragment payload type is not supported by the instruction ABI"
            )
        ...
        if fragment_elements % fragment[op_idx]:
            reject(
                "MFMA operand payload does not form an integral instruction fragment"
            )
    ...
    if (
        payload_elements <= 0
        or not _is_power_of_two(payload_elements)
...
def _mma_component_model(
    ...
    *,
    ...
    if tuple(name for name, _size in out_dims) != ("dim0", "dim1") or any(
        not _is_power_of_two(size) for _name, size in out_dims
    ):
    ...
    visible_fragment = tuple(
        min(logical, physical)
        for logical, physical in zip(shape, fragment, strict=False)
    )
    ...
    input_bases = {
        str(name): tuple(tuple(int(value) for value in basis) for basis in bases)
    ...
    def quotient_basis(basis, source):
        quotient = []
        for value, fragment_extent, tile_extent in zip(
            basis, fragment, tile_shape, strict=False
        ):
    ...
    component_bases = tuple(
        quotient_basis(basis, "component") for basis in register_bases[payload_bits:]
    )
    partition_bases = {
        name: tuple(quotient_basis(basis, name) for basis in input_bases.get(name, ()))
    ...
    partition_masks = basis_masks(
        tuple(basis for bases in partition_bases.values() for basis in bases)
    )

    for dim, extent in enumerate(tile_shape):
        if (
            component_masks[dim] & partition_masks[dim]
    ...
    rows = [
        sum(((column >> row) & 1) << bit for bit, column in enumerate(columns))
        | (1 << (width + row))
        for row in range(width)
    ]
    for pivot in range(width):
        source = next(
            (row for row in range(pivot, width) if rows[row] & (1 << pivot)), None
        )
        if source is None:
    ...
    symbols = {
        name: dsl.sym(f"mma_{name}") for name in ("intra", "lane", "warp", "block")
    }
    ...
    axis_bits = tuple(
        ...
        for dim, mask in enumerate(component_masks)
        for compact_bit, _bit in enumerate(
            bit for bit in range(int(mask).bit_length()) if int(mask) & (1 << bit)
        )
    )
    ...
    facts = tuple(
        ...
        for predicate in _symbolic_range_predicates(
            dsl, symbols[name], 0, int(extent) - 1
        )
    ) + tuple(
        predicate
        for dim, extent in enumerate(grid)
        for predicate in _symbolic_range_predicates(dsl, axes[dim], 0, int(extent) - 1)
    )
    physical = {
        "register": register,
        **{name: symbols[name] for name in ("lane", "warp", "block")},
    }
    logical = _symbolic_layout_formula(
        dsl, linear, {name: physical[name] for name in input_names}
    )
    ...
    def remap_bits(expression, source_bits, destination_bits):
        return sum(
            (
                ...
                for source_bit, destination_bit in zip(
                    source_bits, destination_bits, strict=False
                )
            ),
            dsl.ixs_int(0),
        )
    ...
    goals = [
        ...
        (
            "component_roundtrip",
            dsl.ixs_eq(dsl.floor(register / payload_elements), component),
        ),
        (
            "intra_roundtrip",
            dsl.ixs_eq(dsl.mod(register, payload_elements), symbols["intra"]),
        ),
    ]
    for dim, ((name, logical_extent), fragment_extent, tile_extent) in enumerate(
        zip(out_dims, fragment, tile_shape, strict=False)
    ):
        tile = dsl.floor(logical[name] / fragment_extent)
        fragment_coord = dsl.mod(logical[name], fragment_extent)
        component_bits = tuple(
            bit
            for bit in range(int(component_masks[dim]).bit_length())
            if int(component_masks[dim]) & (1 << bit)
        )
        deposited_component = remap_bits(
            axes[dim], range(len(component_bits)), component_bits
        )
        ...
        goals.extend(
            (
                ...
                (
                    f"{name}_fragment_model",
                    dsl.ixs_eq(fragment_coord, fragment_coords[dim]),
                ),
    ...
    proofs, normalized = dsl.ixs_check(
        (*tuple(goal for _name, goal in goals), component), facts
    )
    ...
    if any(proof is not True for proof in goal_proofs):
        ...
        failed = tuple(
            name
            for (name, _goal), proof in zip(goals, goal_proofs, strict=False)
            if proof is not True
        )
        reject(
            "MFMA canonical component proof returned " f"{status} for goals {failed}"
        )
    return _MmaComponentModel(
        component_count, grid, dsl.ixs_serialize(normalized[len(goals)])
    )
    ...
    *,
    ...
    def component_grid(layout):
        ...
        if (
            ...
            or not isinstance(relation, tuple)
            or not relation
...
def _reshape_linear_basis(basis, source_shape, target_shape):
    flat = 0
...
def _can_reshape_linear_output(source_shape, target_shape):
    return all(
        _is_power_of_two(extent) for extent in (*source_shape, *target_shape)
    ) and _product(source_shape) == _product(target_shape)


def _can_transpose_linear_output(source_shape, target_shape, order):
    return (
        len(order) == len(source_shape)
        and sorted(order) == list(range(len(source_shape)))
        and tuple(source_shape[index] for index in order) == target_shape
    )
...
def relabel_linear_output(linear, target_shape, *, order=None):
    ...
    if order is None:
        if not _can_reshape_linear_output(source_shape, target_shape):
            return None

        def relabel(basis):
            return _reshape_linear_basis(basis, source_shape, target_shape)

    else:
        if not _can_transpose_linear_output(source_shape, target_shape, order):
            return None
...
def _distributed_active_relation(
    ...
    *,
    ...
    if min(lane_width, warp_count, block_count) <= 0:
        reject(
            "TLXW_TYPE_MALFORMED_LAYOUT",
            "distributed ownership requires positive kernel dimensions",
        )
    ...
    if any(not _is_power_of_two(extent) for _name, extent in out_dims):
        reject(
            "TLXW_TYPE_UNSUPPORTED_LAYOUT",
            "replicated ownership requires a binary LinearLayout output domain",
        )
    ...
    if unknown:
        reject(
            "TLXW_TYPE_UNSUPPORTED_LAYOUT",
            f"distributed ownership has unsupported physical dimensions {unknown}",
        )
    ...
    for name in ("register", "lane", "warp", "block"):
        ...
        if not _is_power_of_two(extent):
            reject(
                "TLXW_TYPE_UNSUPPORTED_LAYOUT",
                f"distributed ownership requires a binary {name} domain",
            )
        ...
        if len(bases) > width or any(len(basis) != out_rank for basis in bases):
            reject(
                "TLXW_TYPE_MALFORMED_LAYOUT",
                f"LinearLayout {name} bases do not fit the kernel domain",
            )
        for bit in range(width):
            basis = bases[bit] if bit < len(bases) else (0,) * out_rank
            if any(
                value < 0 or value >= extent
                for value, (_out_name, extent) in zip(basis, out_dims, strict=False)
            ):
                reject(
                    "TLXW_TYPE_MALFORMED_LAYOUT",
                    f"LinearLayout {name} basis exceeds its logical domain",
                )
            shift = 0
            column = 0
            for value, (_out_name, extent) in zip(basis, out_dims, strict=False):
    ...
    if physical_size < logical_size or physical_size % logical_size:
        reject(
            "TLXW_TYPE_MALFORMED_LAYOUT",
            "LinearLayout physical domain does not cover its logical domain",
        )
    replication = physical_size // logical_size
    if not _is_power_of_two(replication):
        reject(
            "TLXW_TYPE_UNSUPPORTED_LAYOUT",
            "distributed ownership requires a binary replication domain",
        )
    ...
    if any(not add_independent(column) for _name, _bit, column in block_columns):
        reject(
            "TLXW_TYPE_UNSUPPORTED_LAYOUT",
            "replicated block ownership cannot be suppressed inside one workgroup",
        )
    nonpivots = []
    ...
    if len(pivots) != logical_width or not linear.is_surjective():
        reject(
            "TLXW_TYPE_UNSUPPORTED_LAYOUT",
            "distributed ownership requires a surjective binary LinearLayout",
        )
    kernel_width = replication.bit_length() - 1
    if len(nonpivots) != kernel_width:
        reject(
            "TLXW_TYPE_MALFORMED_LAYOUT",
            "LinearLayout section does not span the physical replication domain",
        )
    if not nonpivots:
        return None
...
def packet_layout_relations(
    ...
    *,
    ...
    expected = (
        (2, 1) if transform == "join" else ((1, 2) if transform == "split" else (1, 1))
    )
    if (len(sources), len(results)) != expected:
        raise ValueError(
            f"{transform} packet relation requires {expected[0]} source layout(s) "
    ...
    if len(lane_widths) != 1:
        raise ValueError(
            "packet redistribution requires equal source/result lane widths"
        )
    if source_invariant_bits is not None and transform != "identity":
        raise ValueError(
            "source invariants are supported only for identity packet redistribution"
        )
    ...
    if transform == "join":
        ...
        if first.linear_layout != second.linear_layout:
            raise ValueError(
                "packet join requires identical distributed operand layouts"
            )
...
def global_memory_bit_offset_relation(
    ...
    *,
    ...
    element_offset = index_expr.subs(
        {dsl.sym(name): expression for name, expression in logical.items()}
    )
    element_contiguity = int(element_contiguity)
...
def local_memory_bit_offset_relation(
    ...
    *,
    ...
    if (
        not logical_shape
        or len(logical_shape) != len(physical_shape)
        or len(logical_shape) != len(logical_origin)
    ):
        raise ValueError(
            "local memory relation requires matching ranked shapes and origin"
        )
    if any(value <= 0 for value in (*logical_shape, *physical_shape)) or any(
        value < 0 for value in logical_origin
    ):
        raise ValueError(
            "local memory relation requires positive shapes and nonnegative origin"
        )
    if (
        lane_width <= 0
        or warp_count <= 0
    ...
    if any(any(int(value) for value in basis) for basis in block_bases):
        raise ValueError(
            "local memory relation does not support nontrivial block bases"
        )
    expected_outputs = {
        f"dim{dim}": int(extent) for dim, extent in enumerate(logical_shape)
    }
    if {str(name): int(extent) for name, extent in linear.out_dims} != expected_outputs:
        raise ValueError(
            "local memory relation logical dimensions do not match its shape"
        )
    ...
    logical_coords = tuple(
        logical_by_name[f"dim{dim}"] for dim in range(len(logical_shape))
    )
    for coord, extent in zip(logical_coords, logical_shape, strict=False):
        goals.extend((coord >= 0, coord < extent))
    physical_coords = tuple(
        coord + origin
        for coord, origin in zip(logical_coords, logical_origin, strict=False)
    )
    for coord, extent in zip(physical_coords, physical_shape, strict=False):
        goals.extend((coord >= 0, coord < extent))
    origin_is_aligned_tile = all(
        origin % logical == 0 and origin + logical <= physical
        for logical, physical, origin in zip(
            logical_shape, physical_shape, logical_origin, strict=False
        )
    )
    if (
        address_layout is not None
        and not address_layout.prefix_shape
        and origin_is_aligned_tile
        and all(
            _is_power_of_two(extent) for extent in (*logical_shape, *physical_shape)
        )
        and all(
            logical <= physical
            for logical, physical in zip(logical_shape, physical_shape, strict=False)
        )
        ...
        if logical_shape != physical_shape:
            item_linear = LinearLayout.from_bases(
                tuple(
                    (
                        str(name),
                        tuple(tuple(int(value) for value in basis) for basis in bases),
                    )
        ...
        physical_bits = (
            ...
            else _symbolic_layout_bits(dsl, physical_layout, physical_inputs)
        )
        if any(logical_origin):
            ...
            if origin_block:
                raise ValueError(
                    "symbolic local memory tile origin selects another allocation block"
                )
            origin_offset = int(origin_address["offset"].eval({}))
            offset_bits = physical_bits["offset"]
            physical_bits["offset"] = tuple(
                dsl.xor(bit, 1) if origin_offset & (1 << index) else bit
                for index, bit in enumerate(offset_bits)
            )
        goals.append(
            dsl.ixs_eq(
                (
                    mapped["block"]
                    if mapped is not None
                    else _symbolic_bits_to_int(dsl, physical_bits["block"])
                ),
                dsl.ixs_int(0),
            )
        )
        if mapped is not None:
            ...
            for interval, padding in zip(intervals, paddings, strict=False):
...
def memdesc_index_element_offset_relation(
    ...
    *,
    ...
    if (
        not parent_shape
        or not child_shape
        or any(extent <= 0 for extent in (*parent_shape, *child_shape))
    ):
    ...
    if parent_elements % child_elements:
        raise ValueError(
            "memdesc index child shape does not evenly tile its parent shape"
        )
    slot_count = parent_elements // child_elements
    if slot_count <= 0:
        raise ValueError("memdesc index relation requires a nonempty slot domain")

    parent_address, parent_order, parent_intervals, parent_paddings = (
        _shared_address_layout(
            parent_layout,
            parent_shape,
            stage=stage,
            diagnostic=diagnostic,
    ...
    child_address, child_order, child_intervals, child_paddings = (
        _shared_address_layout(
            child_layout,
            child_shape,
            stage=stage,
    ...
    if any(proof is not True for proof in proofs):
        status = "False" if False in proofs else "Unknown"
        raise ValueError(
            "symbolic memdesc index child-translation proof returned " f"{status}"
        )
    return (
...
def shared_allocation_size_bytes(
    ...
    *,
    ...
    padding_extent = sum(
        (logical_extent // interval - int(logical_extent % interval == 0)) * padding
        for interval, padding in zip(intervals, paddings, strict=False)
    )
...
def _packet_relation_blob(
    ...
    *,
    ...
    source_extents = {
        name: linear_layout_in_dim_size(source, name)
        for name in ("register", "lane", "warp", "block")
    }
    result_extents = {
        name: linear_layout_in_dim_size(result, name)
        for name in ("register", "lane", "warp", "block")
    }
    if source_extents["lane"] != result_extents["lane"]:
        raise ValueError(
            "packet redistribution requires equal source/result lane domains"
        )
    if source_extents["block"] != result_extents["block"]:
        raise ValueError(
            "packet redistribution requires equal source/result block domains"
        )
    ...
    if int(destination_components) <= 0 or result_extents["register"] % int(
        destination_components
    ):
    ...
    binary_layouts = all(
        _is_power_of_two(extent) for _name, extent in (*source_dims, *result_dims)
    )
    if transform == "reshape" and not binary_layouts:
        ...
        same_layout = (
            transform == "identity"
            and not structural
            and tuple(source.bases) == tuple(result.bases)
        ...
        if same_layout:
            ...
            raise ValueError(
                "packet redistribution between distinct non-power-of-two layouts "
    ...
    packed = (mapped["block"] * source_items + mapped_item) * source_slots + mapped[
        "register"
    ]
...
def _symbolic_layout_formula(
    ...
    if component_count is not None:
        ...
        if component_count <= 0 or register_count % component_count:
            raise ValueError(
                "packet component count does not split the register domain"
            )
    ...
    return {
        ...
        for (name, size), coordinate in zip(out_dims, coordinates, strict=False)
    }
...
def _symbolic_layout_field_formula(
    ...
    *,
    ...
    if output_weights is None:
        output_weights = {
            name: tuple(1 << bit for bit in range(bit_count))
            for (name, _size), bit_count in zip(out_dims, output_bits, strict=False)
        }
    weights = [tuple(map(int, output_weights[name])) for name, _size in out_dims]
    if any(
        len(values) != bit_count
        for values, bit_count in zip(weights, output_bits, strict=False)
    ):
        return None
    ...
    for name, bases in linear.bases:
        ...
        for input_bit, basis in enumerate(bases):
            ...
            if (
                current is not None
                and current[0] == output
                and current[1] + current[3] == input_bit
        ...
        for output, input_bit, output_bit, width in fields:
            value = (
                source
                if input_bit == 0 and width == len(bases)
                else dsl.mod(
    ...
    for fields in fields_by_output:
        ...
        for field in fields:
            coordinate = (
                dsl.xor(coordinate, field) if combine_with_xor else coordinate + field
            )
        coordinates.append(coordinate)
    return {
        name: coordinate
        for (name, _size), coordinate in zip(out_dims, coordinates, strict=False)
    }
...
def _symbolic_layout_bits(dsl, linear, inputs):
    ...
    bits = [
        [dsl.ixs_int(0) for _ in range(size.bit_length() - 1)]
    ...
    return {
        name: tuple(values)
        for (name, _size), values in zip(out_dims, bits, strict=False)
    }
    ...
    *,
    ...
    if address_layout is None:
        ...
        mapped = _symbolic_layout_formula(
            dsl,
            address_layout.linear_layout,
            {f"dim{dim}": coord for dim, coord in enumerate(inner_coords)},
        )
        outer = dsl.ixs_int(0)
        for coord, extent in zip(
            prefix_coords, address_layout.prefix_shape, strict=False
        ):
...
def _shared_address_layout(
    ...
    *,
    ...
    if layout is not None and kind in {
        ...
        if inner_order:
            if len(inner_order) > len(shape) or sorted(inner_order) != list(
                range(len(inner_order))
            ):
                reject(
                    f"shared layout order {inner_order} cannot be applied to "
                    f"rank-{len(shape)} shape"
                )
            prefix_rank = len(shape) - len(inner_order)
            order = tuple(prefix_rank + dim for dim in inner_order) + tuple(
                reversed(range(prefix_rank))
            )
    if kind == "dense":
    ...
    if component_rank == 0 or component_rank > len(shape):
        reject(
            "shared address map rank does not fit the memdesc shape; "
            f"map={out_dims}, shape={shape}"
        )
    inner_shape = shape[-component_rank:]
    prefix_shape = shape[:-component_rank]
    expected_out_dims = tuple(
        (f"dim{dim}", int(extent)) for dim, extent in enumerate(inner_shape)
    )
    if out_dims != expected_out_dims:
        reject(
            "shared address map output dims do not match the trailing "
    ...
    if kind == "padded_shared":
        intervals = tuple(
            int(value) for value in layout.properties.get("intervals", ())
        )
...
def _complete_packet_physical_dims(linear):
    ...
    if unknown:
        raise ValueError(
            "packet layout has unbound physical input dimensions "
    ...
    if all(name in bases for name in ("register", "lane", "warp", "block")):
        return linear
    names = tuple(bases) + tuple(
        name for name in ("register", "lane", "warp", "block") if name not in bases
    )
    return LinearLayout.from_bases(
...
def _packet_item_linear_layout(
    ...
    *,
    ...
    warp_count = int(warp_count)
    physical_extents = {
        name: linear_layout_in_dim_size(linear, name) for name in _PACKET_PHYSICAL_DIMS
    }
    lane_bits = lane_width.bit_length() - 1
    warp_bits = warp_count.bit_length() - 1
    ...
    item_bases = [
        basis(lane=(1 << bit) if (1 << bit) < physical_extents["lane"] else 0)
        for bit in range(lane_bits)
    ]
    item_bases.extend(
        basis(warp=(1 << bit) if (1 << bit) < physical_extents["warp"] else 0)
        for bit in range(warp_bits)
    )
    adapter_bases = [
        ("slot", slot_bases),
        ("item", item_bases),
    ...
    if preserve_block:
        adapter_bases.append(
            (
                ...
                [
                    basis(block=1 << bit)
                    for bit in range(physical_extents["block"].bit_length() - 1)
                ],
            )
        )
...
def _compose_linear_layouts(inner, outer):
    ...
    outer_bases = {
        str(name): tuple(tuple(map(int, basis)) for basis in values)
        for name, values in outer.bases
    }

    def apply_outer(value):
        result = [0] * len(outer_outputs)
        for input_name, input_value in zip(
            inner_outputs, map(int, value), strict=False
        ):
            for bit, basis in enumerate(outer_bases[input_name]):
                if input_value & (1 << bit):
                    result = [
                        lhs ^ rhs for lhs, rhs in zip(result, basis, strict=False)
                    ]
        return result

    return LinearLayout.from_bases(
        [
            (str(name), [apply_outer(value) for value in values])
            for name, values in inner.bases
        ],
        outer_outputs,
...
def _packet_transform_descriptor(source_shape, result_shape, *, transform, axis, order):
    ...
    if transform == "identity":
        ...
        if len(source_shape) != len(result_shape) or any(
            src not in (1, dst)
            for src, dst in zip(source_shape, result_shape, strict=False)
        ):
            raise ValueError("broadcast layout conversion has incompatible shapes")
        mapped_dims = tuple(
            None if size == 1 else dim for dim, size in enumerate(source_shape)
        )
    elif transform == "expand_dims":
        if (
            axis is None
            or not 0 <= axis < len(result_shape)
        ...
        mapped_dims = tuple(
            None if dim == axis else dim - (dim > axis)
        ...
        if (
            axis is None
            or not 0 <= axis < len(source_shape)
...
def _packet_destination_layout(
    result, source_dims, result_dims, mapped_dims, structural, *, reshape
):
    ...
    for name, values in result.bases:
        bases.append(
            (
                str(name),
                [transform_coordinate(tuple(map(int, basis))) for basis in values],
            )
        )
    if structural:
...
def _packet_layout_columns(linear):
    return tuple(
        ((str(name), bit), tuple(int(component) for component in basis))
        for name, bases in linear.bases
        for bit, basis in enumerate(bases)
    )
...
def _packet_source_equations(linear, columns):
    logical_bits = sum(
        int(extent).bit_length() - 1 for _name, extent in linear.out_dims
    )
...
def _preferred_packet_solution(equations, rhs, source_columns, destination_column):
    ...
    preferred = {
        index: int(destination_column == column)
        for index, (column, _basis) in enumerate(source_columns)
    }
    constraints = []
    ...
    for name in _PACKET_PHYSICAL_DIMS:
        for variable in sorted(
            groups[name], key=lambda index: source_columns[index][0][1], reverse=True
        ):
            if variable in constrained:
                continue
            proposed = [*constraints, (variable, 0)]
            value = int(
                _solve_packet_gf2(equations, rhs, variable_count, proposed) is None
            )
...
def _preferred_packet_relation(
    ...
    *,
    ...
    logical_bits = sum(
        int(extent).bit_length() - 1 for _name, extent in source.out_dims
    )
    ignored_bits = set()
    if source_invariant_bits is not None:
        ...
        if any(bit < 0 or bit >= logical_bits for bit in source_invariant_bits):
            raise ValueError(
                "source invariant logical bit is outside the packet layout"
            )
    ...
    def quotient_coordinate(basis):
        ...
        return sum(
            ((coordinate >> bit) & 1) << quotient_bit
            for quotient_bit, bit in enumerate(active_bits)
        )
    ...
    physical_extents = {
        name: linear_layout_in_dim_size(source, name) for name in _PACKET_PHYSICAL_DIMS
    }
    ...
    for name, bases in destination.bases:
        relation_bases.append(
            (str(name), [physical_coordinate(next(solution)) for _basis in bases])
        )
    return LinearLayout.from_bases(
        relation_bases,
...
def _packet_relation_output_is_direct(relation, name):
    output_dims = tuple(
        (str(output_name), int(extent)) for output_name, extent in relation.out_dims
    )
    output_index = next(
        (
            index
            for index, (output_name, _extent) in enumerate(output_dims)
            if output_name == name
        ),
        None,
    )
...
def _transform_logical_coordinates(
    ...
    *,
    ...
    if reshape:
        ...
        for coordinate, extent in zip(result_values, result_shape, strict=False):
    ...
    return {
        str(name): value
        for (name, _size), value in zip(source_dims, values, strict=False)
    }
