...
def _add_divisibility_facts(source_program, facts):
    ...
    op_by_result = {
        result_id: op for op in source_program.ops for result_id in op.results
    }
...
def _add_assume_facts(source_program, facts):
    ...
    for op in source_program.ops:
        if op.name != "llvm.intr.assume" or len(op.operands) != 1:
            continue
        fact = _assume_compare_fact(
            source_program, op_by_result, op.operands[0], op.index
        )
        if fact is None:
            continue
...
def _derived_ranges_for_op(source_program, facts, op):
    ...
    if (
        ...
        and op.name in _RANGE_PRESERVING_OPS
    ):
...
def _combined_range(facts, value_id, *, source_program=None, use_op_index=None):
    ...
    for fact in facts:
        if fact.kind != "range" or fact.subject_value_id != value_id:
            continue
        if (
            fact.provenance == "llvm.intr.assume"
            and use_op_index is not None
            and not _assumption_dominates_op(
                source_program, fact.source_op_index, use_op_index
            )
        ):
            continue
        found = True
...
def _integer_width(source_type):
    raw_type = (
        source_type.element_type if source_type.kind == "tensor" else source_type.raw
    )
    if raw_type == "index":
        return None
