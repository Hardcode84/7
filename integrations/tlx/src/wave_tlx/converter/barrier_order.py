...
def thread_barrier_issue_order(target_program):
    ...
    def resource_targets(target_value_ids):
        return tuple(
            dict.fromkeys(
                ...
                for resource_target_id in values[
                    int(target_value_id)
                ].resource_target_ids
            )
        )
    ...
    for region in target_program.regions:
        ...
        has_later_ordered_op = _suffix_matches(
            ...
            ops,
            lambda op: (
                op.kind in target_ir.MEMORY_ISSUER_OP_KINDS or _orders_memory_issue(op)
            ),
        )
        ordered_op_ids = []
        ...
        for position, op_id in enumerate(original_op_ids):
            ...
            if op.kind in target_ir.MEMORY_ISSUER_OP_KINDS:
                ...
                needs_frontier_result = (
                    has_later_completion_barrier[position]
                    if requires_completion
                    else has_later_issue_barrier[position]
                )
...
def _lds_data_descendants(target_program):
    ...
    for op in target_program.ops:
        ...
        if op.kind == "local_load":
            ...
            continue
        if op.kind == "for_loop":
            body = target_program.regions[int(op.region_ids[0])]
        ...
        if op.region_ids:
            for region_id in op.region_ids:
                child = target_program.regions[int(region_id)]
                for yielded, result in zip(
                    child.yield_value_ids, op.results, strict=True
                ):
                    connect(yielded, result)
            continue
...
def _lds_consumer_frontiers(region, op_ids, ops, values, lds_derived_values):
    ...
    for position, op_id in enumerate(op_ids):
        ...
        if op.kind == "local_load":
            data_count = int(
                target_ir.attrs_dict(op).get("data_result_count", len(op.results))
            )
            derived_values.update(
                int(result_id) for result_id in op.results[:data_count]
            )
            continue
        ...
        consumer_frontier[:] = [
            value_id for value_id in consumer_frontier if value_id not in consumed
        ]
...
def _thread_structured_memory_issue(target_program):
    ...
    def join_frontier(op_ids, operands, source_op_index, debug_name):
        ...
        domain = (
            ...
            if operands
            else target_ir.EVENT_DOMAIN_EMPTY
        )
        result_id = add_value(
            domain,
            debug_name,
    ...
    def has_barrier(region_id):
        ...
        if region_id not in has_barrier_cache:
            has_barrier_cache[region_id] = any(
                _orders_memory_issue(op)
                or any(has_barrier(child) for child in op.region_ids)
    ...
    def append_barrier_frontier(op_ids, op, frontier):
        ...
        for dependency_id in existing_dependencies:
            ...
            if (
                producer.kind == "issue_token"
                and producer_attrs.get("projection_domain")
                == target_ir.EVENT_DOMAIN_MEMORY_ISSUE
            ):
    ...
    def process_region(region_id, incoming=(), carry_out=False):
        ...
        if region_id in processed:
            fail(
                ...
                STAGE,
                "structured issue region was reached more than once",
        ...
        for position, op_id in enumerate(original_ids):
            ...
            if op.kind == "for_loop" and len(op.region_ids) == 1:
                ...
                arg_id = add_value(
                    target_ir.EVENT_DOMAIN_MEMORY_COMPLETION,
                    f"loop_memory_issue_arg_{op.target_op_id}",
                )
                result_id = add_value(
                    target_ir.EVENT_DOMAIN_MEMORY_COMPLETION,
                ...
                regions[body_id] = replace(
                    body, block_arg_ids=(*body.block_arg_ids, arg_id)
                )
            ...
            if op.kind == "if" and len(op.region_ids) == 2:
                ...
                needs_merge = (
                    bool(frontier)
                    or needs_result
                    or any(has_barrier(branch_id) for branch_id in branch_ids)
                )
                if not needs_merge:
    ...
    return replace(
        target_program, values=tuple(values), ops=tuple(ops), regions=tuple(regions)
    )
...
def _thread_structured_lds_read_completion(target_program):
    ...
    def join_frontier(
        ...
        target_value_ids = tuple(
            dict.fromkeys(int(value_id) for value_id in target_value_ids)
        )
    ...
    def process_region(region_id, incoming_completion_ids=()):
        ...
        region = regions[region_id]
        ordered_op_ids = []
        completion_frontier = list(
            dict.fromkeys(int(value_id) for value_id in incoming_completion_ids)
        )
        ...
        for op_id in region.op_ids:
            ...
            if op.kind == "if" and len(op.region_ids) == 2:
                ...
                for branch_index, branch_id in enumerate(branch_ids):
                    ...
                    yield_id = join_frontier(
                        ...
                        outgoing_ids,
                        op.source_op_index,
                        (f"if_lds_completion_{op.target_op_id}_" f"{branch_index}"),
                    )
                    branch = regions[branch_id]
                    regions[branch_id] = replace(
                        branch,
            ...
            if op.kind == "for_loop" and len(op.region_ids) == 1:
                ...
                needs_carry = bool(completion_frontier) or region_has_local_read(
                    body_region_id
                )
                if not needs_carry:
...
def _orders_memory_issue(op):
    return op.kind == "barrier" and bool(
        target_ir.attrs_dict(op).get("orders_memory_issue", False)
    )
...
def _append_barrier_lds_read_dependencies(op, target_value_ids):
    ...
    target_value_ids = tuple(
        target_value_id
        for target_value_id in dict.fromkeys(
