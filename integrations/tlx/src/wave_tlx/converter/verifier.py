...
from . import boundary, domains, target_ir
...
def _verify_target_layouts(target_program):
    for expected_id, layout in enumerate(target_program.layouts):
        ...
        if layout.linear_layout is not None and not _is_layout_schema_value(
        ...
        def valid_relation(blob):
            return (
                isinstance(blob, tuple)
                and bool(blob)
                and all(type(byte) is int and 0 <= byte <= 255 for byte in blob)
            )
        ...
        if layout.active_relation is not None and (
            layout.linear_layout is None or not valid_relation(layout.active_relation)
        ):
            fail(
                "TLXW_VERIFY_LAYOUT_SCHEMA",
                STAGE,
    ...
    for expected_id, assumption in enumerate(target_program.assumptions):
        ...
        if (
            ...
            fail(
                "TLXW_VERIFY_ASSUMPTION_SCHEMA",
                STAGE,
                f"target assumption {assumption.assumption_id} has an empty " "range",
                fact_id=assumption.assumption_id,
            )
...
def _verify_target_value_types(target_program):
    ...
    for value in target_program.values:
        ...
        for resource_target_id in resources:
            ...
            if (
                target_program.values[resource_target_id].type.representation
                != "memdesc"
            ):
                fail(
                    "TLXW_VERIFY_RESOURCE_TARGET",
        ...
        if (
            value.type.element_type == "i1"
            and value.type.lane_width is not None
            and not mask_representation
        ):
            fail(
        ...
        if value.layout_map_id is not None:
            ...
            if value.type.lane_width is not None and int(value.type.lane_width) != int(
                layout.lane_width
            ):
                fail(
...
def _verify_ops(target_program, source_program):
    ...
    for expected_id, op in enumerate(target_program.ops):
        ...
        for fact_id, target_value_id in zip(
            op.fact_ids, op.fact_target_ids, strict=False
        ):
...
def _verify_memory_issue_order(op, target_program):
    ...
    barrier_issue_operands = (
        tuple(int(operand) for operand in op.operands[-dependency_count:])
        if dependency_count
        else ()
    )
    ...
    ordinary_operands = (
        op.operands[:-dependency_count] if dependency_count else op.operands
    )
...
def _verify_issue_projection_structure(op, target_program, projection_domain):
    ...
    if projection_domain == target_ir.EVENT_DOMAIN_MEMORY_ISSUE:
        for operand in op.operands:
            producers = tuple(
                producer
                for producer in target_program.ops
                if int(operand) in producer.results
            )
            block_arg_regions = tuple(
                region
            ...
            is_memory_issuer = (
                len(producers) == 1
                and producers[0].kind in target_ir.MEMORY_ISSUER_OP_KINDS
            )
            is_structured_completion = (
                len(producers) == 1
                and producers[0].kind
                in {
                    "for_loop",
                    "if",
                    "token_join",
    ...
    if projection_domain == target_ir.EVENT_DOMAIN_BARRIER_ISSUE:
        ...
        if producer.kind != "barrier" or not bool(
            attrs.get("orders_memory_issue", False)
        ):
            fail(
                "TLXW_VERIFY_BARRIER_ORDER_PROVENANCE",
                STAGE,
...
def _target_value_producer(target_program, target_value_id):
    producers = tuple(
        op for op in target_program.ops if int(target_value_id) in op.results
    )
    if len(producers) != 1:
        fail(
            "TLXW_VERIFY_BARRIER_ORDER_PROVENANCE",
...
def _require_value_dominates_op(target_program, target_value_id, consumer):
    ...
    block_arg_regions = tuple(
        region
        for region in target_program.regions
        if target_value_id in region.block_arg_ids
    )
...
def _target_region_parent(target_program, region_id):
    parents = tuple(
        ...
        if int(region_id) in op.region_ids
        for parent_region_id, _ in (
            (_target_op_region_position(target_program, op.target_op_id),)
        )
    )
    if len(parents) > 1:
        fail(
...
def _verify_async_protocol_op(op, target_program, source_program=None):
    ...
    if op.kind == "barrier":
        ...
        if (
            dependency_count not in {0, 1}
            or lds_read_dependency_count < 0
            or issue_dependency_count < 0
            or dependency_count + lds_read_dependency_count + issue_dependency_count
            != len(op.operands)
        ):
            fail(
        ...
        requires_issue_order = int(attrs.get("address_space", 0)) == 31 or bool(
            attrs.get("compiler_membar_barrier", False)
        )
        ...
        if lds_completion_result_count not in {
            0,
            1,
        } or lds_completion_result_count > len(op.results):
            fail(
                "TLXW_VERIFY_ASYNC_PROTOCOL_SEGMENTS",
                STAGE,
    ...
    if op.kind == "async_wait":
        ...
        if min(
            completed_count, retained_count
        ) < 0 or completed_count + retained_count != len(op.operands):
            fail(
                "TLXW_VERIFY_ASYNC_PROTOCOL_SEGMENTS",
                STAGE,
    ...
    if (
        explicit_count not in {0, 1}
        or data_count < 0
        or completion_count not in {0, 1}
...
def _verify_assume_ssa(op, target_program):
    if (
        ...
        or len(set(op.operands)) != len(op.operands)
    ...
    for operand_id, result_id in zip(op.operands, op.results, strict=False):
        ...
        if (
            ...
            or operand.layout_map_id != result.layout_map_id
            or operand.source_value_id != result.source_value_id
            or operand.event_domain != result.event_domain
...
def _verify_mma_component_relation(op, target_program):
    ...
    if (
        ...
        or any(
            ...
            for extent in (m_tiles, n_tiles, k_tiles)
        )
    ):
        fail(
    ...
    for name, grid, target_value_id in domains:
        ...
        layout = (
            target_program.layouts[int(layout_id)]
            if layout_id is not None
            and 0 <= int(layout_id) < len(target_program.layouts)
            else None
        )
        component_count = int(grid[0]) * int(grid[1])
        if (
            layout is None
            or tuple(layout.component_grid or ()) != tuple(grid)
            or not isinstance(layout.component_relation, tuple)
            or not layout.component_relation
...
def _verify_source_semantic_attrs(target_program, op, source_program):
    ...
    source_overflow = boundary.identifier_tokens(
        source_op.attrs.get("overflowFlags", "")
    )
    for flag in ("nsw", "nuw"):
        if flag not in attrs:
            continue
        if type(attrs[flag]) is not bool or (
            attrs[flag] and flag not in source_overflow
        ):
            fail(
                "TLXW_VERIFY_INVENTED_SEMANTICS",
                STAGE,
...
def _verify_memory_edges(op, target_program):
    ...
    if op.kind in {"buffer_load", "buffer_load_to_local", "buffer_store"}:
        ...
        if (
            ...
            or not isinstance(binding_names, tuple)
            or len(binding_names) != binding_count
            or len(set(binding_names)) != binding_count
        ...
        ordinary_count = (
            ...
            + int(bool(attrs.get("has_other", False)))
            if op.kind == "buffer_load"
                ...
                + int(bool(attrs.get("has_other", False)))
                if op.kind == "buffer_load_to_local"
                else 3 + int(bool(attrs.get("has_mask", False)))
            )
        )
        dependency_count = int(attrs.get("barrier_order_dependency_count", 0))
        if op.kind == "buffer_load_to_local":
        ...
        for target_value_id in op.operands[
            ordinary_count : ordinary_count + binding_count
        ]:
            ...
            if binding_type.representation != "scalar" or (
                ...
                and not (
                    isinstance(binding_type.element_type, str)
                    and binding_type.element_type.startswith("i")
                )
            ):
                fail(
                    "TLXW_VERIFY_MEMORY_RELATION",
    ...
    if offset_type.element_type not in {
        ...
        fail(
            "TLXW_VERIFY_MEMORY_EDGE",
            STAGE,
            "memory offset operands must use a SIMD i32 or index " "representation",
            target_op_id=op.target_op_id,
...
def _verify_layout_transform(op, target_program):
    ...
    relations = (
        attrs.get("relations", ()) if op.kind == "split" else (attrs.get("relation"),)
    )
    expected_relation_count = 2 if op.kind == "split" else 1
    if len(relations) != expected_relation_count or any(
        not isinstance(relation, tuple)
        or not relation
    ...
    if (
        any(value.layout_map_id is None for value in values)
    ...
    layouts = tuple(
        target_program.layouts[int(value.layout_map_id)] for value in values
    )
    ...
    if op.kind == "broadcast":
        ...
        valid = len(source_shape) == len(result_shape) and all(
            source in {1, result}
            for source, result in zip(source_shape, result_shape, strict=False)
        )
        ...
        valid = (
            isinstance(axis, int)
            and 0 <= axis < len(result_shape)
            and result_shape[axis] == 1
        ...
        valid = (
            ...
            and result_shape == (*first_shape, 2)
        ...
        valid = (
            bool(source_shape)
            and source_shape[-1] == 2
            and first_shape == second_shape == source_shape[:-1]
        )
    else:
        source_shape, result_shape = shapes
...
def _verify_reduction(op, target_program):
    ...
    if (
        any(value.layout_map_id is None for value in (operand, result))
        or any(
        ...
        or types[0].element_type != types[1].element_type
    ...
    combiner_values = tuple(
        target_program.values[int(target_value_id)]
        for target_value_id in (
            *region.block_arg_ids,
            *region.yield_value_ids,
        )
    )
    if any(
        value.layout_map_id is not None
        or value.type.representation != "simd"
        or value.type.element_type != types[0].element_type
        or value.type.lane_width != types[0].lane_width
        or int(value.type.component_count) != 1
...
def _is_layout_schema_value(value):
    ...
    if isinstance(value, target_ir.TargetLinearLayout):
        if (
            ...
            or len({name for name, _size in value.out_dims}) != len(value.out_dims)
        ...
        if tuple(name for name, _bases in value.bases) != value.in_dims:
            return False
        return all(
            len(basis) == out_rank for _name, bases in value.bases for basis in bases
        )
    if isinstance(value, tuple):
