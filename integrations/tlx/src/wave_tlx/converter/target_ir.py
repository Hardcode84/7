...
EVENT_DOMAIN_LDS_CONSUMER_ORDER = "lds_consumer_order"
...
EVENT_DOMAINS = frozenset(
    {
        ...
        EVENT_DOMAIN_LDS_CONSUMER_ORDER,
        EVENT_DOMAIN_EMPTY,
    }
)
        ...
        *,
        ...
        self.values.append(
            TargetValue(
                ...
                tuple(
                    dict.fromkeys(int(target_id) for target_id in resource_target_ids)
                ),
            )
        )
        if source_value_id is not None:
            self.source_value_targets.setdefault(source_value_id, ())
...
def target_layout_from_converted(layout):
    return TargetLayout(
        ...
        (
            ...
            else _target_layout_property_value(layout.linear_layout)
        ),
        (
            None
            if layout.component_grid is None
            else tuple(int(extent) for extent in layout.component_grid)
        ),
        (
            None
            if layout.component_relation is None
            else tuple(int(byte) for byte in layout.component_relation)
        ),
        (
            None
            if layout.active_relation is None
            else tuple(int(byte) for byte in layout.active_relation)
        ),
    )
        ...
        return TargetLinearLayout(
            ...
            tuple(
                (
                    str(name),
                    tuple(
                        tuple(int(component) for component in basis) for basis in bases
                    ),
                )
