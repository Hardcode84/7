...
def _integer_target_type(target_type):
    ...
    return element_type == "index" or (
        isinstance(element_type, str) and element_type.startswith("i")
    )
...
def _set_binary_relation(builder, view, operation):
    ...
    if operation == "addi":
        ...
        value = (
            quotient if operation == "divui" else dsl.mod(unsigned_lhs, unsigned_rhs)
        )
        bias = 1 << (width - 1)
        expr = dsl.mod(value + bias, 1 << width) - bias
    else:
        return
    builder.value_relations[int(view.result_target_ids[0])] = _IndexRelation(
        expr, _merge_relation_bindings(lhs, rhs)
    )
...
def _logical_relation_transform(builder, source_id, result_id, substitutions):
    ...
    if source is None:
        return
    dsl = layouts.load_wave_dsl()
    expr = source.expr.subs(
        {dsl.sym(name): value for name, value in substitutions.items()}
    )
    builder.value_relations[int(result_id)] = _IndexRelation(expr, source.bindings)


def _logical_view_substitutions(
    source_shape, result_shape, *, transform, axis=None, order=(), selector=None
):
    dsl = layouts.load_wave_dsl()
    ...
    if transform == "expand_dims":
        return {
            f"dim{dim}": result_dims[dim + (dim >= int(axis))]
            for dim in range(len(source_shape))
        }
    if transform == "broadcast":
        return {
            f"dim{dim}": (
                dsl.ixs_int(0) if source_shape[dim] == 1 else result_dims[dim]
            )
            for dim in range(len(source_shape))
        }
    if transform == "reshape":
        flat = dsl.ixs_int(0)
        for coord, extent in zip(result_dims, result_shape, strict=False):
    ...
    if transform == "split":
        substitutions = {
            f"dim{dim}": result_dims[dim] for dim in range(len(result_shape))
        }
        substitutions[f"dim{len(result_shape)}"] = dsl.ixs_int(int(selector))
        return substitutions
    return {
        f"dim{dim}": result_dims[dim]
        for dim in range(min(len(source_shape), len(result_shape)))
    }
    ...
    *,
    ...
    return (
        pointer.base_target_id,
        {
            ...
            "index_binding_names": tuple(
                name for name, _target_id in combined.bindings
            ),
        },
...
def _source_value_use_count(conversion_input, source_value_id):
    ...
    return sum(
        operand_id == source_value_id
        for source_op in conversion_input.ops
        for operand_id in source_op.operands
    )
...
def _build_conversion_input(
    source_program, type_layout_program, fact_program, token_program
):
...
def _seed_kernel_arguments(builder, conversion_input, type_layout_program):
    ...
    for source_value_id in conversion_input.kernel_arg_ids:
        ...
        if converted.type.representation == "uniform_pointer":
            builder.pointer_relations[target_id] = _PointerRelation(
                target_id, _IndexRelation(layouts.load_wave_dsl().ixs_int(0))
            )
...
def _seed_kernel_argument_facts(builder, conversion_input, fact_program):
    for source_value_id in conversion_input.kernel_arg_ids:
        ...
        integer_shaped = target_type.kind in {"scalar", "tensor"} and (
            target_type.element_type == "index"
            or _int_width(target_type.element_type) is not None
        )
        fact_ids = tuple(
            fact_id
            for fact_id in fact_program.by_value.get(source_value_id, ())
            if fact_program.facts[fact_id].source_op_index is None
            and (
                fact_program.facts[fact_id].kind == "range"
                or (fact_program.facts[fact_id].kind == "divisible" and integer_shaped)
            )
        )
        if not fact_ids:
            continue
...
def _refine_assumed_values(
    ...
    *,
    ...
    builder.add_op(
        ...
        results=result_ids,
        fact_ids=fact_ids,
        fact_target_ids=tuple(
            result_by_operand[target_id] for target_id in fact_target_ids
        ),
        source_op_index=source_op_index,
    )
    return result_ids
...
def _resource_target_ids(builder, target_value_ids):
    return tuple(
        dict.fromkeys(
            ...
            for resource_target_id in builder.values[
                int(target_value_id)
            ].resource_target_ids
        )
    )
...
def _convert_constant(builder, view):
    ...
    if type(literal) in {bool, int} and _integer_target_type(
        builder.values[result_target_id].type
    ):
        builder.value_relations[int(result_target_id)] = _IndexRelation(
            layouts.load_wave_dsl().ixs_int(int(literal))
        )
...
def _convert_float_cast(builder, view):
    ...
    if view.op_name == "arith.extf":
        supported = (
            operand_type.element_type in {"f16", "bf16"}
            and result_type.element_type == "f32"
        )
        description = "f16/bf16 to f32"
    else:
        supported = operand_type.element_type == "f32" and result_type.element_type in {
            "f16",
            "bf16",
        }
...
def _convert_mma_packet_float_cast(builder, type_layout_program, op):
    ...
    if (
        operand.type.element_type not in {"f16", "bf16"}
        or result.type.element_type != "f32"
    ):
        fail(
            "TLXW_OP_UNSUPPORTED_FLOAT_CAST",
            STAGE,
...
def _convert_make_range(
    ...
    builder.value_relations[int(result_target_ids[0])] = _IndexRelation(
        int(attrs["start"]) + logical_index
    )
...
def _packet_relation_attrs(
    ...
    *,
    ...
    sources = tuple(
        _layout_for_converted_value(type_layout_program, values[int(value_id)])
        for value_id in source_value_ids
    )
    results = tuple(
        _layout_for_converted_value(type_layout_program, values[int(value_id)])
        for value_id in result_value_ids
    )
...
def _convert_splat(builder, view):
    ...
    if len(view.result_target_ids) == 1 and len(view.operand_target_ids) == 1:
        ...
        if operand.type.representation == "scalar" and _integer_target_type(
            operand.type
        ):
...
def _convert_expand_dims(builder, type_layout_program, op):
    ...
    source_shape = type_layout_program.layouts[
        int(type_layout_program.values[op.operands[0]].layout_map_id)
    ].shape
    ...
    _logical_relation_transform(
        ...
        _logical_view_substitutions(
            source_shape, result_shape, transform="expand_dims", axis=axis
        ),
    )
...
def _convert_broadcast(builder, type_layout_program, op):
    ...
    relation = _packet_relation_attrs(
        type_layout_program, op.operands, op.results, transform="broadcast", op=op
    )[0]
    builder.add_op(
        "broadcast",
        operands=_operand_target_ids(builder, op),
...
def _convert_join(builder, type_layout_program, op):
    ...
    relation = _packet_relation_attrs(
        type_layout_program, op.operands, op.results, transform="join", op=op
    )[0]
    builder.add_op(
        "join",
        operands=_operand_target_ids(builder, op),
    ...
    _join_value_relation(
        builder, _operand_target_ids(builder, op), result_target_ids[0]
    )
...
def _convert_split(builder, type_layout_program, op):
    ...
    relations = _packet_relation_attrs(
        type_layout_program, op.operands, op.results, transform="split", op=op
    )
    builder.add_op(
        "split",
        operands=_operand_target_ids(builder, op),
    ...
    for index, (result_target_id, _relation) in enumerate(
        zip(result_target_ids, relations, strict=False)
    ):
        if source_relation is not None and source_relation.join_branches is not None:
            builder.value_relations[int(result_target_id)] = (
                source_relation.join_branches[index]
            )
            continue
        source_shape = type_layout_program.layouts[
            int(type_layout_program.values[op.operands[0]].layout_map_id)
        ].shape
        result_shape = type_layout_program.layouts[
            int(result_layout_map_ids[index])
        ].shape
        _logical_relation_transform(
            builder,
            source_id,
...
def _convert_warp_vote(builder, type_layout_program, op):
    ...
    result = type_layout_program.values[op.results[0]]
    if (
        predicate.type.representation != "mask"
        or int(predicate.type.component_count) != 1
    ):
        fail(
            "TLXW_OP_WARP_VOTE",
            STAGE,
...
def _convert_if(
    ...
    token_result_target_ids = tuple(
        builder.add_value(
            target_ir.target_type_from_converted(
                type_layout_program.values[
                    _if_token_carry_type_source_value_id(carry)
                ].type
            ),
    ...
    data_result_packet_registers = tuple(
        (
            _mma_packet_registers(
                type_layout_program, type_layout_program.values[source_value_id], op
            )
            if type_layout_program.values[source_value_id].type.representation
            in _MMA_PACKET_REPRESENTATIONS
            else 0
        )
        for source_value_id in op.results
    )
    result_packet_registers = (
        (*data_result_packet_registers, *((0,) * len(token_carries)))
        if data_result_packet_registers
        else ()
    )
    builder.add_op(
        "if",
    ...
    _replace_source_targets(
        ...
        tuple(
            ...
            for carry, token_result_target_id in zip(
                token_carries, token_result_target_ids, strict=False
            )
...
def _convert_for(
    ...
    token_result_target_ids = tuple(
        builder.add_value(
            target_ir.target_type_from_converted(
                type_layout_program.values[
                    _loop_token_carry_type_source_value_id(carry)
                ].type
            ),
    ...
    induction_fact_ids = tuple(
        fact_id
        for fact_id in fact_program.by_value.get(source_region.block_arg_ids[0], ())
    ...
    issue_dependencies = _loop_async_issue_dependencies(
        tuple(
            carry for carry, _token_block_arg_target_id in token_issue_dependency_pairs
        ),
        tuple(
    ...
    body_conversion_input = replace(
        ...
        async_wait_entry_dependency_target_ids_by_value_id={
            ...
            **{
                int(carry.init_source_value_id): tuple(
                    dict.fromkeys(
                        (
                            *(
                ...
                if (
                    carry.init_source_value_id is not None and not carry.readiness_carry
                )
            },
        },
    )
...
def _loop_token_yield_target_id(
    ...
    token_target_id = builder.add_value(
        target_ir.target_type_from_converted(
            type_layout_program.values[init_source_value_id].type
        ),
...
def _loop_async_issue_dependencies(token_carries, token_block_arg_target_ids):
    dependencies_by_op = {}
    for carry, token_block_arg_target_id in zip(
        token_carries, token_block_arg_target_ids, strict=False
    ):
        for op_index in carry.issue_dependency_op_indices:
            existing = dependencies_by_op.setdefault(op_index, ())
...
def _convert_memdesc_index(
    ...
    memdesc = _memdesc_info(conversion_input, op.results[0], op)
    if (
        memdesc.element_byte_width is None
        or parent_memdesc.element_byte_width != memdesc.element_byte_width
    ...
    parent_shape = tuple(
        int(dim) for dim in (parent_memdesc.alloc_shape or parent_memdesc.shape)
    )
    ...
    try:
        element_offset_relation, slot_count = (
            layouts.memdesc_index_element_offset_relation(
                parent_layout,
                child_layout,
                parent_shape,
                child_shape,
    ...
    builder.add_op(
        ...
        results=result_target_ids,
        attrs={
            "element_offset_relation": tuple(
                int(value) for value in element_offset_relation
            ),
...
def _convert_memdesc_trans(builder, type_layout_program, op):
    ...
    result = type_layout_program.values[op.results[0]]
    if (
        operand.type.representation != "memdesc"
        or result.type.representation != "memdesc"
    ):
        fail(
            "TLXW_OP_MEMDESC_TRANS",
            STAGE,
...
def _local_component_store_plan_attrs(destination_plan):
    return {
        "destination_bit_offset_relation": tuple(
            int(value) for value in destination_plan["bit_offset_relation"]
        ),
    }
...
def _local_tensor_access_attrs(
    ...
    lane_width = int(
        tensor_value.type.lane_width
        or tensor_layout.lane_width
...
def _buffer_load_mma_packet_result_value_attrs(
    type_layout_program, loaded, has_other, op
):
...
def _convert_buffer_load(
    builder, conversion_input, type_layout_program, fact_program, op
):
...
def _convert_load(builder, conversion_input, type_layout_program, op):
    ...
    builder.add_op(
        ...
        attrs={
            ...
            "mask_mode": (
                "exec_where" if fields["mask_value_id"] is not None else "none"
            ),
            **relation_attrs,
        },
        layout_map_ids=result_layout_map_ids,
...
def _convert_local_load(builder, conversion_input, type_layout_program, op):
    ...
    if (
        ...
        and type_layout_program.values[token_value_id].type.representation != "token"
    ):
        fail(
            "TLXW_OP_LOCAL_LOAD",
            STAGE,
    ...
    dependency_source_value_ids = (
        (token_value_id,)
        if token_value_id is not None
        else conversion_input.implicit_wait_readiness_value_ids_by_op.get(
            op.index,
            (),
        )
    )
    dependency_target_ids = tuple(
        dict.fromkeys(
            _single_source_target(builder, source_value_id, op)
            for source_value_id in dependency_source_value_ids
        )
    )
    target_operands = (
...
def _convert_dot(builder, conversion_input, type_layout_program, op):
    ...
    lane_width = int(
        ...
        or lhs.type.lane_width
        or rhs.type.lane_width
        or acc.type.lane_width
        or 64
    )
    result_target_ids, result_layout_map_ids = _declare_results(
        builder,
...
def _convert_dot_scaled(builder, conversion_input, type_layout_program, op):
    ...
    acc = type_layout_program.values[op.operands[2]]
    lhs_scale = (
        type_layout_program.values[op.operands[3]] if len(op.operands) == 5 else None
    )
    rhs_scale = (
        type_layout_program.values[op.operands[4]] if len(op.operands) == 5 else None
    )
    result = type_layout_program.values[op.results[0]]
    result_layout = type_layout_program.layouts[int(result.layout_map_id)]
    if result_layout.kind != "amd_mfma":
...
def _convert_mma_packet_truncf(builder, type_layout_program, op):
    ...
    operand = type_layout_program.values[op.operands[0]]
    result = type_layout_program.values[op.results[0]]
    if operand.type.element_type != "f32" or result.type.element_type not in {
        "f16",
        "bf16",
    }:
        fail(
...
def _require_yield_layouts(
    ...
    for index, (yielded_source_value_id, result_value_id) in enumerate(
        zip(yielded_source_value_ids, result_value_ids, strict=False)
    ):
        _require_same_layout_except_element_type(
            type_layout_program,
...
def _require_for_iter_layouts(
    ...
    for index, (init_value_id, result_value_id) in enumerate(
        zip(init_value_ids, result_value_ids, strict=False)
    ):
        _require_same_layout_except_element_type(
            type_layout_program,
    ...
    for index, (block_arg_value_id, result_value_id) in enumerate(
        zip(block_arg_value_ids, result_value_ids, strict=False)
    ):
        _require_same_layout_except_element_type(
            type_layout_program,
...
def _convert_async_commit_group(
    builder, type_layout_program, token_groups_by_commit, op
):
    ...
    issue_delay_cycles = _int_attr_or_default(
        op.attrs, "tlx.async_issue_delay_cycles", 0
    )
...
def _convert_async_wait(
    ...
    retained_group_operand_ids = tuple(
        dict.fromkeys(
            ...
            if (
                conversion_input.token_groups_by_id[group_id].token_value_id is not None
                and _implicit_group_is_reachable(
                    conversion_input,
    ...
    wait_token_ids = (
        ...
        else _implicit_wait_token_ids(conversion_input, node, op)
    )
    ...
    entry_group_operand_ids = tuple(
        ...
        if (
            target_id not in group_operand_ids
            and target_id not in retained_group_operand_ids
        )
    )
    ...
    builder.add_op(
        ...
        attrs={
            ...
            "waited_group_ids": tuple(
                int(group_id) for group_id in node.waited_group_ids
            ),
            "retained_group_ids": tuple(
                int(group_id) for group_id in node.retained_group_ids
            ),
            "completed_group_dependency_count": len(group_operand_ids)
            + len(entry_group_operand_ids),
...
def _convert_reduce(
    ...
    block_arg_target_ids = tuple(
        builder.add_value(
            target_ir.target_type_from_converted(
                combiner_type_layout_program.values[source_value_id].type
            ),
            source_value_id=source_value_id,
...
def _convert_select(
    ...
    if result_type.representation != "scalar" and all(
        relation is not None
        for relation in (condition_relation, true_relation, false_relation)
    ):
        expr = (
            condition_relation.expr * true_relation.expr
            + (1 - condition_relation.expr) * false_relation.expr
        )
        builder.value_relations[int(result_target_id)] = _IndexRelation(
            expr,
...
def _pack_mma_packet_edge(
    ...
    builder.add_op(
        ...
        layout_map_ids=(
            () if result.layout_map_id is None else (result.layout_map_id,)
        ),
        source_op_index=op.index,
    )
...
_SIMPLE_OP_CONVERTERS = {
    ...
    **dict.fromkeys(_BINARY_OPS, _convert_binary),
    **dict.fromkeys(_FLOAT_BINARY_OPS, _convert_float_binary),
    **dict.fromkeys(_FLOAT_UNARY_OPS, _convert_float_unary),
    **dict.fromkeys(_FLOAT_CAST_OPS, _convert_float_cast),
...
def _convert_barrier(builder, conversion_input, op):
    ...
    attrs = {
        ...
        "dependency_count": len(readiness_target_ids),
...
def _fact_ids_by_source_op(fact_program):
    ...
    for fact in fact_program.facts:
        if fact.source_op_index is None:
            continue
        result.setdefault(fact.source_op_index, ())
...
def _source_token_crosses_if_branch_path(
    conversion_input, commit_op_index, wait_op_index
):
...
def _compute_memdesc_view_infos(
    ...
    def base_view(value_id):
        ...
        physical_shape = tuple(
            int(dim) for dim in (memdesc.alloc_shape or memdesc.shape)
        )
    ...
    def require_same_memdesc_type(lhs_id, rhs_id, op, description):
        ...
        mismatches = tuple(
            field for field in fields if getattr(lhs, field) != getattr(rhs, field)
        )
        if mismatches:
            fail(
    ...
    for op in ops:
        ...
        memdesc_results = tuple(
            result_id for result_id in op.results if result_id in memdescs
        )
        ...
        if op.name == "ttg.memdesc_reshape":
            ...
            if (
                parent.element_type != child.element_type
            ...
            child_physical_shape = tuple(
                int(dim) for dim in (child.alloc_shape or child.shape)
            )
            if _product(parent.shape) != _product(child.shape) or _product(
                parent_view.physical_shape
            ) != _product(child_physical_shape):
                fail(
                    "TLXW_OP_MEMDESC_RESHAPE",
                    STAGE,
        ...
        if op.name == "ttg.memdesc_subslice":
            ...
            child_type = source_values[result_id].type
            if (
                parent_type.encoding != child_type.encoding
                or parent_type.memory_space != child_type.memory_space
                or parent_type.mutable != child_type.mutable
            ):
                fail(
            ...
            for offset, child_extent, parent_extent in zip(
                offsets, child_shape, parent_shape, strict=False
            ):
                if (
                    int(offset) < 0
                    or int(child_extent) <= 0
                    or int(offset) + int(child_extent) > int(parent_extent)
                ):
                    fail(
                        "TLXW_OP_MEMDESC_SUBSLICE",
                        STAGE,
            ...
            logical_origin = tuple(
                ...
                for origin, offset in zip(
                    parent_view.logical_origin, offsets, strict=False
                )
            )
            result[result_id] = MemdescViewInfo(
                result_id,
...
def _buffer_store_fields(op):
    ...
    return {
        ...
        "base_value_id": op.operands[base_index],
        "offset_value_id": op.operands[offset_index],
        "boundary_check_value_id": (
            op.operands[boundary_index] if int(segments[3]) else None
        ),
...
def _local_component_store_plan(
    ...
    *,
    ...
    if (
        ...
        or offset_layout.linear_layout is None
    ...
    if (
        ...
        or layouts.linear_layout_in_dim_size(linear, "block") != 1
...
def _require_dot_operand_layout(layout, op_idx, op):
    if layout.kind != "dot_operand" or int(layout.properties.get("op_idx", -1)) != int(
        op_idx
    ):
        fail(
            "TLXW_OP_DOT",
...
def _require_dot_operand_parent_layout(operand_layout, result_layout, op_idx, op):
    ...
    if (
        parent_kind == result_layout.kind
        and parent_properties == result_layout.properties
    ):
        return
    fail(
        "TLXW_OP_DOT",
