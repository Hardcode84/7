"""Explicit token graph for the TLX Wave converter."""
...
class TokenProgram:
    ...
    def users_for_value(self, value_id):
        return tuple(
            self.nodes[node_id] for node_id in self.users_by_value_id.get(value_id, ())
        )
...
def _implicit_wait_readiness_by_op(source_program):
        ...
        for op_index in region.op_indices:
                    ...
                    loop_carry_candidates.append(
                        (
                            LoopTokenCarry(
                                ...
                                init_source_value_id=(
                                    None
                                    if not wait_value_ids
                                    else int(wait_value_ids[0])
                                ),
                                yield_source_value_id=(
                                    None
                                    if not body_wait_value_ids
                                    else int(body_wait_value_ids[0])
                                ),
            ...
            if op.name == "scf.if" and len(op.region_ids) == 2:
                ...
                then_wait_value_ids, else_wait_value_ids = (
                    branch_wait_value_ids for branch_wait_value_ids, _ in branch_results
                )
                if then_wait_value_ids == else_wait_value_ids:
                    wait_value_ids = then_wait_value_ids
                    continue
                then_source_value_id = (
                    None if not then_wait_value_ids else int(then_wait_value_ids[0])
                )
                else_source_value_id = (
                    None if not else_wait_value_ids else int(else_wait_value_ids[0])
                )
    ...
    while changed:
        changed = False
        for candidate_index, (carry, body_op_indices) in enumerate(
            loop_carry_candidates
        ):
            ...
            if not any(
                op_index in body_op_indices for op_index in entry_uses
        ...
        for candidate_index, (carry, branch_op_indices) in enumerate(
            if_carry_candidates
        ):
            ...
            if not any(
                op_index not in branch_op_indices
                for value_id in branch_value_ids
    ...
    return (
        dependencies,
        {op_index: tuple(carries) for op_index, carries in loop_carries.items()},
        {op_index: tuple(carries) for op_index, carries in if_carries.items()},
    )


def _is_implicit_wait_ready_load(op):
    return (
        op.name == "ttg.local_load"
...
def _consumes_implicit_wait_readiness(op):
    ...
    return _is_implicit_wait_ready_load(op) or op.name in {
        "ttg.barrier",
        "rocdl.s.barrier",
    }
...
def _committed_queue_before_op(nodes, groups, op_index):
    ...
    for token_op_index in sorted(set(groups_by_commit) | set(nodes_by_op)):
        ...
        if (
            node is not None
            and node.op_name == "ttg.async_wait"
            and not node.input_token_ids
        ):
            committed_queue = _remove_waited_groups(
                committed_queue, node.waited_group_ids
            )
...
def _loop_body_end_queue(initial_queue, nodes, groups, body_op_indices):
    ...
    nodes_by_op = {
        node.op_index: node for node in nodes if node.op_index in body_op_indices
    }
...
def _loop_async_queue_carries(
    ...
    for slot in sorted(needed_slots):
        ...
        yield_token_id = (
            None if slot >= len(end_queue) else end_queue[slot].token_value_id
        )
        ...
        carries.append(
            LoopTokenCarry(
                ...
                cumulative_completion=(
                    yield_token_id is not None and yield_token_id != init_token_id
                ),
            )
        )
...
def _loop_waited_external_tokens(
    op, body_op_indices, nodes, groups_by_id, initial_token_ids
):
    waited_tokens = []
    for node in sorted(nodes, key=lambda node: node.op_index):
        ...
        for group_id in node.waited_group_ids:
            ...
            if (
                group.commit_op_index in body_op_indices
                or token_id not in initial_token_ids
            ):
                continue
...
def _externally_waited_body_tokens(nodes, groups_by_id, body_op_indices):
    ...
    for node in sorted(nodes, key=lambda node: node.op_index):
        ...
        for group_id in node.waited_group_ids:
            group = groups_by_id[group_id]
            if (
                group.commit_op_index not in body_op_indices
                or group.token_value_id is None
            ):
                continue
...
def _operand_or_none(op, index):
    return (
        op.operands[index] if index is not None and index < len(op.operands) else None
    )
