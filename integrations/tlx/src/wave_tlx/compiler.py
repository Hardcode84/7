...
class TLXWaveBackend(amd_compiler.HIPBackend):
    ...
    def parse_options(self, opts) -> Any:
        opts = dict(opts)
        enable_multi_wave_specialize = opts.pop(
            "tlx_wave_enable_multi_wave_specialize", None
        )
        if enable_multi_wave_specialize is None:
            enable_multi_wave_specialize = os.environ.get(
                _ENABLE_MULTI_WAVE_SPECIALIZE_ENV
            )
    ...
    def make_wave(src, metadata, options):
        ...
        compiler_membar_barriers = tuple(
            op for op in _barrier_ops(src) if op not in existing_barriers
        )
    ...
    def add_stages(self, stages, options, language):
        if language != Language.TRITON:
            raise NotImplementedError(
                "tlx_wave scaffold currently supports only Triton/TLX language input"
            )
    ...
    def hash(self):
        return (
            f"{self.target}:stage15-symbolic-arithmetic-contract:"
            f"backend-source-sha256={backend_source_sha256()}:"
...
def _validate_staged_converter_output(output, options):
    ...
    if output.target_program.contract.enable_fp_fusion != bool(
        ...
        raise ValueError(
            "tlx_wave staged converter saw inconsistent FP-fusion semantics"
        )
...
def _populate_staged_converter_metadata(metadata, output, options, wave_opt):
    ...
    metadata["tlx_wave_num_async_waits"] = sum(
        1 for op in target_program.ops if op.kind == "async_wait"
    )
