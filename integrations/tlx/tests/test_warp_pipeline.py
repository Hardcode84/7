...
def _warmup_gfx9_v9_backend(
    ...
    *,
    ...
    with knobs.cache.scope(), knobs.runtime.scope():
        ...
        b_strides = (1, k)
        c_strides = c.stride()
        return tutorial.v9_beyond_hotloop.warmup(
            a,
            b,
            c,
            m,
...
def _run_wave_promote_buffer_to_machine(wave_artifact):
    from wave_tlx.wave_bridge_tools import _wave_machine_pipeline_args, _wave_opt

    wave_opt = _wave_opt()
    result = subprocess.run(
        [wave_opt, "-", *_wave_machine_pipeline_args(wave_opt, "gfx950")],
        input=wave_artifact,
        text=True,
        capture_output=True,
        check=False,
...
def _run_wave_to_amdgpu_asm(wave_artifact):
    from wave_tlx.wave_bridge_tools import _wave_tool

    wave_translate = _wave_tool("wave-translate")
    asm = subprocess.run(
        [wave_translate, "--wave-to-amdgpu-asm", "-"],
        input=wave_artifact,
        text=True,
        capture_output=True,
        check=False,
...
def test_gfx9_v9_tlx_wave_warmup_lowers_to_machine(monkeypatch, tmp_path):
    ...
    assert machine.count("waveamdmachine.ds_load_b128") == 72
    assert machine.count("waveamdmachine.buffer_store_b128") == 16
    assert "waveamdmachine.buffer_store_b64" not in machine
    assert machine.count("waveamdmachine.v_cndmask_b32_tuple") == 80
    assert amd_asm.count("v_cndmask_b32") == 16
    assert machine.count("waveamdmachine.exec_if") == 16
    ...
    assert asm.count("buffer_store") == amd_asm.count("buffer_store") == 16
    assert asm.count("v_cvt_pk_f16_f32") == amd_asm.count("v_cvt_pk_f16_f32") == 64
    assert asm.count("s_barrier") == amd_asm.count("s_barrier") + 2 == 11
...
def test_gfx9_v9_tlx_wave_hot_loop_waits_are_not_full_drains(monkeypatch, tmp_path):
    ...
    assert wave.count("wave.barrier") == 13
    assert asm.count("s_barrier") == amd_asm.count("s_barrier") + 2 == 21
