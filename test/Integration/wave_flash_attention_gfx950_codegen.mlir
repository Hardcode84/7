...
// RUN: %python %S/../../examples/wave/flash_attention_gfx950.py \
// RUN:   --batch=1 --heads=1 --sequence=256 --xcds=1 --waves=8 \
// RUN:   --qk-max-abs=1 2>/dev/null \
// RUN:   | FileCheck %s --check-prefix=IR-BOUNDED
...
// RUN: not %python %S/../../examples/wave/flash_attention_gfx950.py \
// RUN:   --batch=1 --heads=1 --sequence=256 --xcds=1 --waves=8 \
// RUN:   --qk-max-abs=2 2>&1 \
// RUN:   | FileCheck %s --check-prefix=INVALID-QK-BOUND
//
...
// IR-8W: wave.barrier [[K_READ_8]],
//
// IR-BOUNDED: func.func @flash_attention_bf16_gfx950
// IR-BOUNDED-NOT: wave.ballot
//
// INVALID-SEQUENCE: sequence must be a multiple of 256; got 384
// INVALID-XCDS: xcds must be 1, 2, 4, or 8; got 3
// INVALID-QK-BOUND: qk_max_abs fixed reference exceeds BF16 range
//
...
// CHECK-4W: v_add_f32
// CHECK-4W: v_rcp_f32
// CHECK-4W: .amdhsa_next_free_vgpr 404
