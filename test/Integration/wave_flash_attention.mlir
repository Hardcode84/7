...
// XFAIL: host-supports-amdgpu-gfx950
//
...
// RUN:   | FileCheck %s
// RUN: %python %S/../../examples/wave/flash_attention.py --chip=%chip \
// RUN:   --block-m=8 --block-n=8 --seq-n=8 --head-dim=32 --seed=11 --compare-cpu \
// RUN:   | FileCheck %s
// RUN: %python %S/../../examples/wave/flash_attention.py --chip=%chip \
// RUN:   --block-m=8 --block-n=8 --seq-n=16 --head-dim=32 --seed=11 --compare-cpu \
// RUN:   | FileCheck %s
...
// RUN:   --variants=scheduled --skip-hw | FileCheck %s --check-prefix=SCHEDULED
// RUN: %python %S/../../tools/wave-fa-calibrate/wave-fa-calibrate.py --chip=%chip \
// RUN:   --block-m=8 --block-n=8 --seq-n=16 --head-dim=32 --seed=11 \
// RUN:   --variants=scheduled --skip-hw | FileCheck %s --check-prefix=SCHEDULED
//
