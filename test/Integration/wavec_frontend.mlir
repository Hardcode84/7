...
# RUN: ln -sfn %wave_llvm_tools_dir/ld.lld %t.root/llvm-install/bin/ld.lld
...
# RUN: mkdir -p %t.embedded/wave-build/bin %t.embedded/llvm-install/bin
# RUN: cp %wave_obj_root/bin/wavec %t.embedded/wave-build/bin/wavec
# RUN: ln -sfn %wave_llvm_tools_dir/ld.lld %t.embedded/llvm-install/bin/ld.lld
# RUN: env -u WAVE_LLVM_TOOLS_DIR \
# RUN:   WAVE_PIPELINES_DIR=%wave_obj_root/share/wave-mlir/pipelines \
# RUN:   PATH=%t.empty %t.embedded/wave-build/bin/wavec -c \
# RUN:   -o %t.embedded.hsaco %S/../../wavec/test/e2e/good/saxpy.wave
# RUN: %llvm_readelf --notes %t.embedded.hsaco | FileCheck %s --check-prefix=NOTE
...
# RUN: mkdir -p %t.no-lld/root/bin
# RUN: cp %wave_obj_root/bin/wavec %t.no-lld/root/bin/wavec
# RUN: not env -u WAVE_LLVM_TOOLS_DIR \
# RUN:   WAVE_PIPELINES_DIR=%wave_obj_root/share/wave-mlir/pipelines \
# RUN:   PATH=%t.empty %t.no-lld/root/bin/wavec -c -o %t.no-lld.hsaco \
