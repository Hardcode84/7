...
// CHECK-SAME: carries(%{{.*}}, %[[INIT]] : !waveamdmachine.reg<sgpr, 1>, !waveamdmachine.mem.token, !waveamdmachine.reg<sgpr, 1>)
module attributes {waveamdmachine.target = "amdgcn-amd-amdhsa--gfx1100"} {
func.func @cyclic_scalar_offset_carry_constant_init(
    %a: !wave.ptr<#wave.global, i32>) -> !wave.mem.token attributes {wave.kernel} {
  ...
  %observe_region_2 = scf.for %i = %c0 to %c8 step %c1 iter_args(%observe_carry_3 = %observe_seed_1) -> (!wave.mem.token) : i32  {
    ...
    %observed_store_1 = wave.store %v -> %p after %t
...
// CHECK-SAME: %[[NEXT]]
module attributes {waveamdmachine.target = "amdgcn-amd-amdhsa--gfx1100"} {
func.func @extracted_strided_kloop(%a: !wave.ptr<#wave.global, f16>, %n: i32) -> !wave.mem.token attributes {wave.kernel} {
  %c0 = arith.constant 0 : i32
  %c1 = arith.constant 1 : i32
...
func.func @extracted_scaled_nested_binding(
    %a: !wave.ptr<#wave.global, f16>, %base_raw: i32, %n: i32) -> !wave.mem.token attributes {wave.kernel} {
  %c0 = arith.constant 0 : i32
  %c1 = arith.constant 1 : i32
  ...
  %observe_region_10 = scf.for %i = %c0 to %n step %c1 iter_args(%observe_carry_11 = %observe_seed_9) -> (!wave.mem.token) : i32  {
    ...
    %v, %t = wave.load %p
        : (!wave.simd<!wave.ptr<#wave.global, f16>, 32>)
        -> (!wave.simd<vector<8xi32>, 32>, !wave.mem.token)
    %observed_store_3 = wave.store %v -> %p
        : (!wave.simd<vector<8xi32>, 32>,
           !wave.simd<!wave.ptr<#wave.global, f16>, 32>) -> !wave.mem.token
    %observe_join_12 = wave.join %observe_carry_11, %observed_store_3 : !wave.mem.token, !wave.mem.token -> !wave.mem.token
...
func.func @extracted_nested_symbolic_stride(
  ...
  %wi = wave.workitem_id 0 : !wave.simd<i32, 32>
  %observe_seed_17 = wave.token : !wave.mem.token
  %observe_region_18 = scf.for %i = %c0 to %n step %c1 iter_args(%observe_carry_19 = %observe_seed_17) -> (!wave.mem.token) : i32  {
    %observe_seed_13 = wave.token : !wave.mem.token
    %observe_region_14 = scf.for %j = %c1 to %m step %c1 iter_args(%observe_carry_15 = %observe_seed_13) -> (!wave.mem.token) : i32  {
      ...
      %v, %t = wave.load %p
          : (!wave.simd<!wave.ptr<#wave.global, f16>, 32>)
          -> (!wave.simd<vector<8xi32>, 32>, !wave.mem.token)
      %observed_store_4 = wave.store %v -> %p
          : (!wave.simd<vector<8xi32>, 32>,
             !wave.simd<!wave.ptr<#wave.global, f16>, 32>) -> !wave.mem.token
      %observe_join_16 = wave.join %observe_carry_15, %observed_store_4 : !wave.mem.token, !wave.mem.token -> !wave.mem.token
      scf.yield %observe_join_16 : !wave.mem.token
    }
    %observe_join_20 = wave.join %observe_carry_19, %observe_region_14 : !wave.mem.token, !wave.mem.token -> !wave.mem.token
    scf.yield %observe_join_20 : !wave.mem.token
  }
  return %observe_region_18 : !wave.mem.token
}
}

// CHECK-LABEL: func.func @shared_dma_wide_iv_stride_pow2
// CHECK: %[[LOOP:.*]] = waveamdmachine.uniform_loop
// CHECK: ^bb0(%[[IV:[^:]+]]: !waveamdmachine.reg<sgpr, 2>, %{{[^:]+}}: !waveamdmachine.mem.token):
...
func.func @shared_dma_wide_iv_stride_pow2(
    %in: !wave.ptr<#wave.global, i32>) -> !wave.mem.token attributes {wave.kernel, wave.lds_size = 512 : i64} {
  ...
  %res, %observe_region_22 = scf.for %i = %c0 to %ub step %c1
      iter_args(%dst = %lds, %observe_carry_23 = %observe_seed_21) -> (!wave.ptr<#wave.shared, i32>, !wave.mem.token) {
    ...
    %observe_join_24 = wave.join %observe_carry_23, %tok : !wave.mem.token, !wave.mem.token -> !wave.mem.token
    scf.yield %next, %observe_join_24 : !wave.ptr<#wave.shared, i32>, !wave.mem.token
  }
...
// CHECK: waveamdmachine.global_load_lds_b128
module attributes {waveamdmachine.target = "amdgcn-amd-amdhsa--gfx950"} {
func.func @shared_dma_wide_iv_stride_mul(%in: !wave.ptr<#wave.global, i32>) -> !wave.mem.token attributes {wave.kernel, wave.lds_size = 512 : i64} {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  ...
  %src = wave.ptr_add %in, %wi
      ...
      -> !wave.simd<!wave.ptr<#wave.global, i32>, 64>
  %lds = wave.shared_memory_base : !wave.ptr<#wave.shared, i32>
  %observe_seed_25 = wave.token : !wave.mem.token
  %res, %observe_region_26 = scf.for %i = %c0 to %ub step %c1
      iter_args(%dst = %lds, %observe_carry_27 = %observe_seed_25) -> (!wave.ptr<#wave.shared, i32>, !wave.mem.token) {
    %tok0 = wave.token : !wave.mem.token
    %tok = waveamd.dma_load_lds %src -> %dst after %tok0 {bytes = 16 : i64}
        : (!wave.simd<!wave.ptr<#wave.global, i32>, 64>,
           !wave.ptr<#wave.shared, i32>, !wave.mem.token) -> !wave.mem.token
    %next = wave.ptr_add %dst, %c3
        : !wave.ptr<#wave.shared, i32>, index -> !wave.ptr<#wave.shared, i32>
    %observe_join_28 = wave.join %observe_carry_27, %tok : !wave.mem.token, !wave.mem.token -> !wave.mem.token
    scf.yield %next, %observe_join_28 : !wave.ptr<#wave.shared, i32>, !wave.mem.token
  }
  return %observe_region_26 : !wave.mem.token
}
}
...
func.func @reject_shared_symbolic_pointer_carry_machine(
    %stride_raw: i32, %n: i32) -> !wave.mem.token attributes {wave.kernel, wave.lds_size = 4096 : i64} {
  %c0 = arith.constant 0 : i32
  %c1 = arith.constant 1 : i32
  ...
  %observe_seed_29 = wave.token : !wave.mem.token
  %observe_region_30 = scf.for %i = %c0 to %n step %c1 iter_args(%observe_carry_31 = %observe_seed_29) -> (!wave.mem.token) : i32  {
    ...
    %observed_store_5 = wave.store %v -> %p after %t
        : (!wave.simd<vector<4xi32>, 64>,
           !wave.simd<!wave.ptr<#wave.shared, i8>, 64>, !wave.mem.token)
        -> !wave.mem.token
    %observe_join_32 = wave.join %observe_carry_31, %observed_store_5 : !wave.mem.token, !wave.mem.token -> !wave.mem.token
    scf.yield %observe_join_32 : !wave.mem.token
  }
  return %observe_region_30 : !wave.mem.token
}
}
...
func.func @drop_dead_simd_offset_carries_machine(
    %a: !wave.ptr<#wave.global, i32>, %n: i32) -> !wave.mem.token attributes {wave.kernel} {
  %c1 = arith.constant 1 : i32
  ...
  return %unused#2 : !wave.mem.token
}
}
