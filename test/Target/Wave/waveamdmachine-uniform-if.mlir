...
// RUN: wave-opt --split-input-file %s --pass-pipeline='builtin.module(waveamd-to-machine,waveamd-abi-lowering,waveamd-insert-ticket-waits,transform-preload-library{transform-library-paths=%wave_pipelines},transform-interpreter{entry-point=waveamd_regalloc_transform_loop},waveamd-insert-hazard-waits)' | wave-translate --wave-to-amdgpu-asm --split-input-file - | FileCheck %s --check-prefix=ASM
// RUN: wave-opt --split-input-file %s --pass-pipeline='builtin.module(waveamd-to-machine,waveamd-abi-lowering,waveamd-insert-ticket-waits,transform-preload-library{transform-library-paths=%wave_pipelines},transform-interpreter{entry-point=waveamd_regalloc_transform_loop},waveamd-insert-hazard-waits)' | wave-translate --wave-to-amdgpu-asm --split-input-file - | llvm-mc -triple=amdgcn-amd-amdhsa -mcpu=gfx1100 -filetype=obj -o /dev/null
