...
// An index expression shared through a value-preserving `wave.assume` may have
// compound range facts that ixsimpl cannot reduce to symbol intervals. Preserve
// the narrow materialization selected by IntegerRangeAnalysis so buffer offsets
// do not become register pairs.
...
// A symbolic expression operand can already be a selected machine immediate.
// Treat that binding as static when it is used as a modulo divisor.
