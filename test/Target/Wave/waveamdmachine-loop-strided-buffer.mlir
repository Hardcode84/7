...
func.func @strided_buf(%a: !wave.ptr<f16, #wave.global>, %n: i32, %r: i32)
  ...
  %p0 = wave.ptr_add %buf, %off : !wave.ptr<f16, #waveamd.buffer>, !wave.simd<index, 32>
  ...
  scf.for %i = %c0 to %n_bounded step %c1 iter_args(%q = %p0)
    ...
    wave.store %v -> %q : (!wave.simd<vector<8xi32>, 32>, !wave.simd<!wave.ptr<f16, #waveamd.buffer>, 32>) -> !wave.mem.token
    %nq = wave.ptr_add %q, %c16 : !wave.simd<!wave.ptr<f16, #waveamd.buffer>, 32>, i32
        -> !wave.simd<!wave.ptr<f16, #waveamd.buffer>, 32>
    scf.yield %nq : !wave.simd<!wave.ptr<f16, #waveamd.buffer>, 32>
  }
  return
}
}
