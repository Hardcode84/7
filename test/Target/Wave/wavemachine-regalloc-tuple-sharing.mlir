...
// CHECK: %[[E:.+]] = wavemachine.v_mov_b32_tuple {{.*}} -> !wavemachine.reg<vgpr, 1, 5>
// CHECK: %[[F:.+]] = wavemachine.v_mov_b32_tuple {{.*}} -> !wavemachine.reg<vgpr, 1, 6>
// CHECK: %[[G:.+]] = wavemachine.v_mov_b32_tuple {{.*}} -> !wavemachine.reg<vgpr, 1, 7>
// CHECK: %[[T1:.+]] = wavemachine.tuple_from_elements %[[A]], %[[B]], %[[C]], %[[D]]
// CHECK-SAME: -> !wavemachine.reg<vgpr, 4, 0>
// CHECK: %[[ACPY:.+]] = wavemachine.v_mov_b32_tuple %[[A]] {registers = 1 : i64} : (!wavemachine.reg<vgpr, 1, 0>) -> !wavemachine.reg<vgpr, 1, 4>
// CHECK: %{{.+}} = wavemachine.tuple_from_elements %[[ACPY]], %[[E]], %[[F]], %[[G]]
...
func.func @shared_operand_gets_copy() {
  ...
  %t2 = wavemachine.tuple_from_elements %a, %e, %f, %g
      : (!wavemachine.reg<vgpr, 1>, !wavemachine.reg<vgpr, 1>,
         !wavemachine.reg<vgpr, 1>, !wavemachine.reg<vgpr, 1>)
        -> !wavemachine.reg<vgpr, 4>
  return
...
// CHECK: %[[A:.+]] = wavemachine.v_mov_b32_tuple {{.*}} -> !wavemachine.reg<vgpr, 1, 0>
// CHECK: %[[A1:.+]] = wavemachine.v_mov_b32_tuple %[[A]] {registers = 1 : i64} : {{.*}} -> !wavemachine.reg<vgpr, 1, 1>
// CHECK: %[[A2:.+]] = wavemachine.v_mov_b32_tuple %[[A]] {registers = 1 : i64} : {{.*}} -> !wavemachine.reg<vgpr, 1, 2>
// CHECK: %[[A3:.+]] = wavemachine.v_mov_b32_tuple %[[A]] {registers = 1 : i64} : {{.*}} -> !wavemachine.reg<vgpr, 1, 3>
// CHECK: %{{.+}} = wavemachine.tuple_from_elements %[[A]], %[[A1]], %[[A2]], %[[A3]]
// CHECK-SAME: -> !wavemachine.reg<vgpr, 4, 0>
func.func @broadcast_same_value_per_slot() {
  %zero = wavemachine.imm 0 : !wavemachine.imm
  %a = wavemachine.v_mov_b32_tuple %zero {registers = 1 : i64} : (!wavemachine.imm) -> !wavemachine.reg<vgpr, 1>
  %t = wavemachine.tuple_from_elements %a, %a, %a, %a
      : (!wavemachine.reg<vgpr, 1>, !wavemachine.reg<vgpr, 1>,
         !wavemachine.reg<vgpr, 1>, !wavemachine.reg<vgpr, 1>)
        -> !wavemachine.reg<vgpr, 4>
  return
...
func.func @round_trip_no_copies() {
  ...
  %e:4 = wavemachine.tuple_to_elements %a
      : (!wavemachine.reg<vgpr, 4>) -> (!wavemachine.reg<vgpr, 1>,
                                        ...
                                        !wavemachine.reg<vgpr, 1>,
                                        !wavemachine.reg<vgpr, 1>)
  %b = wavemachine.tuple_from_elements %e#0, %e#1, %e#2, %e#3
      : (!wavemachine.reg<vgpr, 1>, !wavemachine.reg<vgpr, 1>,
         !wavemachine.reg<vgpr, 1>, !wavemachine.reg<vgpr, 1>)
        -> !wavemachine.reg<vgpr, 4>
  return
...
// CHECK: %[[A:.+]] = wavemachine.v_mov_b32_tuple {{.*}} -> !wavemachine.reg<vgpr, 4, 0>
// CHECK: %[[E:.+]]:4 = wavemachine.tuple_to_elements %[[A]]
// CHECK: %[[C3:.+]] = wavemachine.v_mov_b32_tuple %[[E]]#3 {registers = 1 : i64}
// CHECK: %[[C2:.+]] = wavemachine.v_mov_b32_tuple %[[E]]#2 {registers = 1 : i64}
// CHECK: %[[C1:.+]] = wavemachine.v_mov_b32_tuple %[[E]]#1 {registers = 1 : i64}
// CHECK: %[[C0:.+]] = wavemachine.v_mov_b32_tuple %[[E]]#0 {registers = 1 : i64}
// CHECK: %{{.+}} = wavemachine.tuple_from_elements %[[C3]], %[[C2]], %[[C1]], %[[C0]]
func.func @shuffle_needs_copies() {
  ...
  %e:4 = wavemachine.tuple_to_elements %a
      : (!wavemachine.reg<vgpr, 4>) -> (!wavemachine.reg<vgpr, 1>,
                                        ...
                                        !wavemachine.reg<vgpr, 1>,
                                        !wavemachine.reg<vgpr, 1>)
  %b = wavemachine.tuple_from_elements %e#3, %e#2, %e#1, %e#0
      : (!wavemachine.reg<vgpr, 1>, !wavemachine.reg<vgpr, 1>,
         !wavemachine.reg<vgpr, 1>, !wavemachine.reg<vgpr, 1>)
        -> !wavemachine.reg<vgpr, 4>
  return
