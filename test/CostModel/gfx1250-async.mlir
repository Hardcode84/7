...
// CLASS: op=waveamdmachine.global_load_async_to_lds_b8 class=WriteVMEM fu=VMEM latency=320 counter_latency=320 issues=1
// TIMELINE: counter_drained cycle=320 fu=VMEM counter=async op=waveamdmachine.global_load_async_to_lds_b8
// WAIT: query op_index=3 cycle=2 op=waveamdmachine.s_waitcnt_split stall=waitcnt cycles=318 components=waitcnt:318
// WAIT: commit op_index=3 issue=320 next=320
// WAIT: query op_index=4 cycle=320 op=waveamdmachine.s_waitcnt_split stall=waitcnt cycles=1 components=waitcnt:1
// WAIT: commit op_index=4 issue=321 next=321
