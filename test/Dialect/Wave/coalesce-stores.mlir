...
// CHECK-SAME: ([[OUT:%.*]]: !wave.ptr<f16, #wave.global>, [[A:%.*]]: !wave.simd<f16, 32>, [[B:%.*]]: !wave.simd<f16, 32>)
func.func @store_pair(%out: !wave.ptr<f16, #wave.global>,
                      %a: !wave.simd<f16, 32>,
                      %b: !wave.simd<f16, 32>)
  ...
  // CHECK: wave.store [[PACK]] -> [[OUT]] : (!wave.simd<vector<2xf16>, 32>, !wave.ptr<f16, #wave.global>) -> !wave.mem.token
...
// CHECK-SAME: ([[OUT:%.*]]: !wave.ptr<i16, #wave.global>, [[A0:%.*]]: !wave.simd<i16, 32>, [[A1:%.*]]: !wave.simd<i16, 32>, [[A2:%.*]]: !wave.simd<i16, 32>, [[A3:%.*]]: !wave.simd<i16, 32>)
func.func @store_quad_recursive(%out: !wave.ptr<i16, #wave.global>,
                                %a0: !wave.simd<i16, 32>,
                                %a1: !wave.simd<i16, 32>,
                                %a2: !wave.simd<i16, 32>,
                                %a3: !wave.simd<i16, 32>)
  ...
  %t0 = wave.store %a0 -> %out
      : (!wave.simd<i16, 32>, !wave.ptr<i16, #wave.global>)
      -> !wave.mem.token
...
// CHECK-SAME: ([[OUT:%.*]]: !wave.ptr<f16, #wave.global>, [[LO:%.*]]: !wave.simd<f16, 32>, [[HI:%.*]]: !wave.simd<f16, 32>)
func.func @reversed_source_order(%out: !wave.ptr<f16, #wave.global>,
                                 %lo: !wave.simd<f16, 32>,
                                 %hi: !wave.simd<f16, 32>)
    attributes {wave.kernel} {
  %c1 = arith.constant 1 : i32
...
// CHECK-SAME: ([[OUT:%.*]]: !wave.ptr<f32, #wave.global>, [[A:%.*]]: !wave.simd<f32, 32>, [[B:%.*]]: !wave.simd<f32, 32>)
func.func @symbolic_delta_and_token_rewrite(%out: !wave.ptr<f32, #wave.global>,
                                            %a: !wave.simd<f32, 32>,
                                            %b: !wave.simd<f32, 32>)
    attributes {wave.kernel} {
  ...
  // CHECK: [[TOK:%.*]] = wave.store [[PACK]] -> %{{.*}} : (!wave.simd<vector<2xf32>, 32>, !wave.simd<!wave.ptr<f32, #wave.global>, 32>) -> !wave.mem.token
...
// CHECK-SAME: ([[OUT:%.*]]: !wave.ptr<bf16, #wave.global>, [[A:%.*]]: !wave.simd<bf16, 32>, [[B:%.*]]: !wave.simd<bf16, 32>)
func.func @bf16_pair(%out: !wave.ptr<bf16, #wave.global>,
                     %a: !wave.simd<bf16, 32>,
                     %b: !wave.simd<bf16, 32>)
    attributes {wave.kernel} {
  %c1 = arith.constant 1 : i32
  ...
  // CHECK: wave.store [[PACK]] -> [[OUT]]
  %t0 = wave.store %a -> %out
      : (!wave.simd<bf16, 32>, !wave.ptr<bf16, #wave.global>)
      -> !wave.mem.token
  %t1 = wave.store %b -> %p1 after %t0
      : (!wave.simd<bf16, 32>, !wave.ptr<bf16, #wave.global>,
         !wave.mem.token)
...
// CHECK-SAME: ([[OUT:%.*]]: !wave.ptr<f16, #wave.global>, [[OTHER:%.*]]: !wave.ptr<f32, #wave.global>, [[A:%.*]]: !wave.simd<f16, 32>, [[B:%.*]]: !wave.simd<f16, 32>, [[M:%.*]]: !wave.simd<f32, 32>)
func.func @intervening_independent_store_can_cross(%out: !wave.ptr<f16, #wave.global>,
                                                   %other: !wave.ptr<f32, #wave.global>,
                                                   %a: !wave.simd<f16, 32>,
                                                   %b: !wave.simd<f16, 32>,
                                                   %m: !wave.simd<f32, 32>)
    attributes {wave.kernel} {
...
// CHECK: wave.store
// CHECK: wave.load
func.func @escaped_intermediate_token_stays(%out: !wave.ptr<f16, #wave.global>,
                                            %a: !wave.simd<f16, 32>,
                                            %b: !wave.simd<f16, 32>)
    attributes {wave.kernel} {
  %c1 = arith.constant 1 : i32
...
// CHECK: wave.store
// CHECK: wave.store
func.func @forked_store_token_stays(%out: !wave.ptr<f16, #wave.global>,
                                    %other: !wave.ptr<f32, #wave.global>,
                                    %a: !wave.simd<f16, 32>,
                                    %b: !wave.simd<f16, 32>,
                                    %m: !wave.simd<f32, 32>)
...
// CHECK: wave.store
// CHECK: wave.store
func.func @intervening_store_token_chain_stays(%out: !wave.ptr<f16, #wave.global>,
                                               %other: !wave.ptr<f32, #wave.global>,
                                               %a: !wave.simd<f16, 32>,
                                               %b: !wave.simd<f16, 32>,
                                               %m: !wave.simd<f32, 32>)
...
// CHECK: wave.store
// CHECK: wave.store
func.func @non_adjacent_address_stays(%out: !wave.ptr<f16, #wave.global>,
                                      %a: !wave.simd<f16, 32>,
                                      %b: !wave.simd<f16, 32>)
    attributes {wave.kernel} {
  %c2 = arith.constant 2 : i32
...
// CHECK: wave.store
// CHECK: wave.store
func.func @mismatched_type_stays(%out: !wave.ptr<f16, #wave.global>,
                                 %a: !wave.simd<f16, 32>,
                                 %b: !wave.simd<i16, 32>)
    attributes {wave.kernel} {
  %c1 = arith.constant 1 : i32
  ...
  %t0 = wave.store %a -> %out
      ...
      -> !wave.mem.token
  %t1 = wave.store %b -> %p1 after %t0
      : (!wave.simd<i16, 32>, !wave.ptr<f16, #wave.global>,
         !wave.mem.token)
      -> !wave.mem.token
  return
}
...
// CHECK: wave.store
// CHECK: wave.store
func.func @mismatched_symbol_binding_stays(%out: !wave.ptr<f32, #wave.global>,
                                           %a: !wave.simd<i32, 32>,
                                           %b: !wave.simd<i32, 32>,
                                           %v0: !wave.simd<f32, 32>,
                                           %v1: !wave.simd<f32, 32>)
