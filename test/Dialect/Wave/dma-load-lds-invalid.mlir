...
func.func @dma_load_lds_source_simd_non_ptr(%bad: !wave.simd<i32, 32>,
  ...
  %tok = waveamd.dma_load_lds %bad -> %lds after %t {bytes = 4 : i64}
      : (!wave.simd<i32, 32>, !wave.ptr<i32, #wave.shared>, !wave.mem.token) -> !wave.mem.token
  return
}

// -----

func.func @dma_load_lds_source_shared(%bad: !wave.simd<!wave.ptr<i32, #wave.shared>, 32>,
                                       %lds: !wave.ptr<i32, #wave.shared>,
                                       %t: !wave.mem.token) {
...
func.func @dma_load_lds_dest_element_type(%src: !wave.simd<!wave.ptr<i32, #wave.global>, 32>,
                                           %lds: !wave.ptr<i64, #wave.shared>,
                                           %t: !wave.mem.token) {
  // expected-error @below {{destination pointer element type must be i32}}
  %tok = waveamd.dma_load_lds %src -> %lds after %t {bytes = 4 : i64}
      : (!wave.simd<!wave.ptr<i32, #wave.global>, 32>,
         !wave.ptr<i64, #wave.shared>, !wave.mem.token) -> !wave.mem.token
  return
}

// -----

func.func @make_buffer_base_not_global(%p: !wave.ptr<i32, #wave.shared>, %r: i32) {
  // expected-error @below {{base must be a global wave pointer}}
  %b = waveamd.make_buffer %p, %r : !wave.ptr<i32, #wave.shared>, i32 -> !wave.ptr<i32, #waveamd.buffer>
  return
}

// -----

func.func @make_buffer_result_not_buffer(%p: !wave.ptr<i32, #wave.global>, %r: i32) {
  // expected-error @below {{result must be a waveamd buffer pointer}}
  %b = waveamd.make_buffer %p, %r : !wave.ptr<i32, #wave.global>, i32 -> !wave.ptr<i32, #wave.global>
  return
}

// -----

func.func @make_buffer_element_mismatch(%p: !wave.ptr<i32, #wave.global>, %r: i32) {
  // expected-error @below {{base and result element types must match}}
  %b = waveamd.make_buffer %p, %r : !wave.ptr<i32, #wave.global>, i32 -> !wave.ptr<i64, #waveamd.buffer>
  return
}

// -----

func.func @make_buffer_range_not_i32(%p: !wave.ptr<i32, #wave.global>, %r: i64) {
  // expected-error @below {{range must be i32 bytes}}
  %b = waveamd.make_buffer %p, %r : !wave.ptr<i32, #wave.global>, i64 -> !wave.ptr<i32, #waveamd.buffer>
  return
}
