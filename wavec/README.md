...
(lexer/parser/sema) links only libc; the lowering bridge and driver link the
in-tree Wave and MLIR targets.
...
../tools/wavec/wavec.cpp   driver, backend entry point, and file I/O
...
The same driver runs the backend in-process for assembly or HSACO:

```sh
build/bin/wavec -S --offload-arch=gfx1100 wavec/test/golden/saxpy.wave
build/bin/wavec -c --offload-arch=gfx1100 -o saxpy.hsaco \
  wavec/test/golden/saxpy.wave
```

`--pipeline-file` overrides the backend transform library. `--target` accepts a
full target attribute when `--offload-arch` is not enough.
...
| `shared_memory_base<T>([K])` | `wave.shared_memory_base {offset=K}` : `shared T*` | yes |
...
- Default output is textual Wave IR. `-S` runs the Wave-to-AMDGPU backend;
  `-c` assembles and links HSACO. Integration tests cover both paths.
- The lowering bridge still renders text for the driver to parse. Direct
  AST-to-MLIR construction remains follow-up work.
