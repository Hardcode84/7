...
Default calibration inputs are random. `--all-ones` is a debug/probe mode, not
the baseline. Large checked runs compute a CPU reference; use `--no-check` for
perf sweeps and keep correctness checks to smaller smoke shapes.
