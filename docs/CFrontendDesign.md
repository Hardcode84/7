...
```c
shared half *lds_a = lds_base<half>(0);       // #wave.shared; kernel: [[amdgpu_lds_size(N)]]
...
```
`lds_base<T>(byteOffset)` returns a uniform `shared T*` into the kernel LDS
arena; the kernel declares arena size with `[[amdgpu_lds_size(N)]]` (->
`wave.lds_size`).
...
| `wavec/test/golden/mlir/lds_roundtrip.mlir` | LDS tokens | `lds_base : #wave.shared`, `wave.lds_size` attr, `store ... after %t` -> `barrier %t -> token` -> `load ... after` |
